...
PALETTE = {
    ...
    # Extracted from reference screenshot palette (quantized/median samples)
    'bg': '#000810',
    'panel': '#000818',
    'surface': '#001018',
    'surface_alt': '#001020',
    ...
    'muted': '#c8c8c8',
    'button': '#0030e0',
    'button_secondary': '#001020',
    'button_neutral': '#303030',
    'border': '#001030',
    'panel_border': '#001030',
    'success': '#00d800',
    'danger': '#f81810',
...
class SECFinancialSystem:
    def __init__(self, root):
        ...
        self.translate_technical_var = tk.BooleanVar(master=self.root, value=True)
        ...
        try:
            ...
            try:
                from modules.ml_trainer import initialize_training_system
                with contextlib.redirect_stdout(io.StringIO()):
    ...
    def _init_ui(self):
        ...
        brand_row.pack(side='top', anchor='w', pady=(0, 0))
        ...
        self.header_title_shadow = tk.Label(
            title_wrap,
            text=title_text,
            font=(FONTS['header'][0], max(20, int(FONTS['header'][1]) - 2), 'bold'),
            bg=PALETTE['nav_bg'],
            fg='#000000',
            anchor='w',
        )
        ...
        self.header_subtitle_shadow = tk.Label(
            subtitle_wrap,
            text=subtitle_text,
            font=(FONTS['subtitle'][0], max(9, int(FONTS['subtitle'][1]) - 1)),
            bg=PALETTE['nav_bg'],
            fg='#000000',
            anchor='w',
        )
    ...
    def _configure_typography(self):
        ...
        FONTS.update({
            ...
            'tree': (arabic_family, 12),
            'caption': (arabic_family, 10, 'bold'),
            'mono': (mono_family, 10),
        })
    ...
    def _init_financial_term_translations(self):
        ...
        add_term('Shares used in computing earnings per share:', 'الأسهم المستخدمة في احتساب ربحية السهم:')
        add_term('Cash dividends declared per share (in dollars per share)', 'التوزيعات النقدية المعلنة لكل سهم (بالدولار لكل سهم)')
        add_term('Cash dividends declared per share', 'التوزيعات النقدية المعلنة لكل سهم')
        add_term('Dividends declared per share', 'التوزيعات المعلنة لكل سهم')
        add_term('Dividends paid', 'التوزيعات المدفوعة')
        add_term('Dividends Paid', 'التوزيعات المدفوعة')
        ...
        add_term('Cash and cash equivalents, at carrying value', 'النقد وما يعادله، بالقيمة الدفترية')
        add_term('Cash and cash equivalents, beginning of period', 'النقد وما يعادله، بداية الفترة')
        add_term('Cash and cash equivalents, end of period', 'النقد وما يعادله، نهاية الفترة')
        add_term('Net cash provided by operating activities', 'صافي النقد من الأنشطة التشغيلية')
        add_term('Net cash used in operating activities', 'صافي النقد المستخدم في الأنشطة التشغيلية')
        add_term('Net cash provided by used in operating activities', 'صافي النقد من الأنشطة التشغيلية')
        add_term('Net cash provided by used in investing activities', 'صافي النقد من الأنشطة الاستثمارية')
        add_term('Net cash provided by used in financing activities', 'صافي النقد من الأنشطة التمويلية')
    ...
    def _on_language_changed(self, _event=None):
        ...
        if code == 'ar' and hasattr(self, 'translate_technical_var'):
            self.translate_technical_var.set(True)
        self._apply_ui_text_fixes()
    ...
    def _on_workspace_search_focus(self):
        try:
            if self.workspace_search_var.get().strip().lower() == "search 50,000+ companies":
                self.workspace_search_var.set("")
                self.workspace_search_entry.configure(fg=PALETTE['text'])
        except Exception:
            pass

    def _on_workspace_search_blur(self):
        try:
            if not self.workspace_search_var.get().strip():
                self.workspace_search_var.set("Search 50,000+ companies")
                self.workspace_search_entry.configure(fg=PALETTE['header_muted'])
        except Exception:
            pass
    ...
    def _set_table_density_from_label(self, _event=None):
        label = str(self.table_density_var.get() or "").strip().lower()
        if "compact" in label or "مضغوط" in label:
            density = "compact"
        else:
            density = "comfortable"
        self._apply_table_density(density)

    def _apply_table_density(self, density="comfortable"):
        rowheight = 30 if density == "compact" else 40
        font = FONTS['normal'] if density == "compact" else FONTS['tree']
        style = ttk.Style(self.root)
        style.configure("Treeview", rowheight=rowheight, font=font)
        style.configure("Treeview.Heading", font=FONTS['label'])
        try:
            self.workspace_hint_label.configure(
                text="وضع عرض مضغوط" if (self.current_lang == 'ar' and density == "compact") else ("Compact view" if density == "compact" else "")
            )
        except Exception:
            pass

    def _sync_workspace_nav_state(self):
        current_key = self._current_notebook_tab_key()
    ...
    def _build_right(self, parent):
        ...
        self.workspace_search_var = tk.StringVar(value="Search 50,000+ companies")
        self.workspace_search_entry = tk.Entry(
            search_shell,
            textvariable=self.workspace_search_var,
            relief='flat',
            bd=0,
            font=FONTS['normal'],
        )
        self._style_entry_widget(self.workspace_search_entry)
        self.workspace_search_entry.configure(
            bg=PALETTE['surface'],
            fg=PALETTE['header_muted'],
            highlightthickness=0,
            insertbackground=PALETTE['text'],
        )
        self.workspace_search_entry.pack(side='left', fill='x', expand=True, padx=(0, 10), pady=6)
        self.workspace_search_entry.bind('<FocusIn>', lambda _e: self._on_workspace_search_focus())
        self.workspace_search_entry.bind('<FocusOut>', lambda _e: self._on_workspace_search_blur())

        quick_actions = tk.Frame(search_row, bg=PALETTE['surface_alt'])
        quick_actions.pack(side='right')
        self.quick_overview_btn = tk.Button(
            quick_actions,
            text="Overview",
            font=FONTS['normal'],
            command=lambda: self._select_notebook_tab(self.raw_tab),
        )
        self._style_tk_button(self.quick_overview_btn, 'secondary')
        self.quick_overview_btn.pack(side='left', padx=(0, 6))
        self.quick_financials_btn = tk.Button(
            quick_actions,
            text="Financials",
            font=FONTS['normal'],
            command=lambda: self._select_notebook_tab(self.ratios_tab),
        )
        self._style_tk_button(self.quick_financials_btn, 'secondary')
        self.quick_financials_btn.pack(side='left', padx=(0, 6))
        self.quick_ai_btn = tk.Button(
            quick_actions,
            text="AI",
            font=FONTS['normal'],
            command=lambda: self._select_notebook_tab(self.ai_analysis_tab),
        )
        self._style_tk_button(self.quick_ai_btn, 'primary')
        self.quick_ai_btn.pack(side='left')
        # Search box is intentionally removed (unused UX surface).
        try:
        ...
        top_company_entry = tk.Entry(ops_row, font=FONTS['normal'], width=26)
        self._style_entry_widget(top_company_entry)
        top_company_entry.pack(side='left', padx=(0, 6))
        top_company_entry.insert(0, "AAPL, MSFT")
        top_company_entry.bind('<Return>', lambda _e: self._add_companies())
        ...
        self.company_selector = ttk.Combobox(
            ops_row,
            textvariable=self.company_selector_var,
            state='readonly',
            values=[],
            width=18,
        )
        ...
        progress_row = tk.Frame(meta_row, bg=PALETTE['surface_alt'])
        progress_row.pack(side='left', padx=(8, 8))
        self.progress_label = tk.Label(
            progress_row,
            text="",
            bg=PALETTE['surface_alt'],
        ...
        self.progress_bar = ttk.Progressbar(progress_row, mode='indeterminate', length=140)
        self.progress_bar.pack(side='top', fill='x')
        self.loading_indicator_canvas = tk.Canvas(
            progress_row, width=140, height=4, bg=PALETTE['surface'], highlightthickness=0
        )
        self.loading_indicator_canvas.pack(side='top', fill='x', pady=(1, 0))
        self._loading_dot = self.loading_indicator_canvas.create_oval(
            2, 2, 10, 10, fill='#28a745', outline='#28a745', state='hidden'
        )
        self._loading_anim_job = None
        self._loading_anim_pos = 2
        self._loading_anim_dir = 1
        ...
        density_wrap = tk.Frame(meta_row, bg=PALETTE['surface_alt'])
        density_wrap.pack(side='left', padx=(0, 8))
        tk.Label(
            density_wrap,
            text="Density",
            bg=PALETTE['surface_alt'],
        ...
        self.table_density_var = tk.StringVar(value="Comfortable")
        self.table_density_combo = ttk.Combobox(
            density_wrap,
            textvariable=self.table_density_var,
            state='readonly',
            values=["Comfortable", "Compact"],
            width=12,
        )
        self.table_density_combo.pack(side='left')
        self.table_density_combo.bind('<<ComboboxSelected>>', self._set_table_density_from_label)
        ...
        notebook_shell.pack(side='left', fill='both', expand=True)
        ...
        self.learning_tab = tk.Frame(self.notebook, bg=PALETTE['panel'])
        ...
        self.notebook.add(self.learning_tab, text=self._t('tab_learning'))
        self._lazy_tab_dirty = {
            'comparison': True,
            'forecast': True,
            'ai': True,
        }
        ...
        self._bind_treeview_scroll(self.raw_tree)
        ...
        self._bind_treeview_scroll(self.ratios_tree)
        ...
        self._bind_treeview_scroll(self.strat_tree)
        ...
        self.v2_engine_btn = tk.Button(
            tools,
            text=self._t('tool_v2_engine'),
            bg='#0f766e',
            fg='white',
            relief='flat',
            font=FONTS['button'],
            command=self.launch_professional_engine_v2,
        )
        self.v2_engine_btn.pack(side='left', padx=6)
    ...
    def _refresh_company_selector(self):
        if not hasattr(self, 'company_selector'):
            return
        try:
            names = list(self.companies_listbox.get(0, tk.END))
        except Exception:
            names = []
        self.company_selector.configure(values=names)
        current_ticker = str(((self.current_data or {}).get('company_info', {}) or {}).get('ticker') or '').strip()
        if current_ticker and current_ticker in names:
            self.company_selector_var.set(current_ticker)
        elif names and (self.company_selector_var.get() not in names):
            self.company_selector_var.set(names[0])
        elif not names:
            self.company_selector_var.set("")

    def _on_company_selector_change(self, _event=None):
        chosen = str(self.company_selector_var.get() or '').strip()
        if not chosen:
            return
        names = list(self.companies_listbox.get(0, tk.END))
        ...
        try:
            ...
            self.companies_listbox.selection_set(idx)
            self.companies_listbox.activate(idx)
    ...
    def _install_tk_text_decoder(self) -> None:
        ...
        def insert_with_decoder(widget, parent, index, iid=None, **kw):
            ...
            if isinstance(vals, (list, tuple)):
                # Keep row values untouched. Data rows are prepared explicitly
                # by the caller and should not pass through a generic UI
                # translation layer, otherwise raw labels and exported parity
                # can be corrupted.
            ...
            try:
                if not getattr(widget, "_sec_zebra_configured", False):
                    widget.tag_configure("zebra_even", background=PALETTE['surface_soft'], foreground=PALETTE['text'])
                    widget.tag_configure("zebra_odd", background=PALETTE['surface'], foreground=PALETTE['text'])
                    widget.tag_configure("header", background=PALETTE['surface_alt'], foreground=PALETTE['text'])
    ...
    def _apply_ui_text_fixes(self) -> None:
        ...
        try:
            ...
            if hasattr(self, 'v2_engine_btn'):
                self.v2_engine_btn.config(text=self._t('tool_v2_engine'))
            ...
            if hasattr(self, 'quick_overview_btn'):
                self.quick_overview_btn.config(text='نظرة عامة' if self.current_lang == 'ar' else 'Overview')
            if hasattr(self, 'quick_financials_btn'):
                self.quick_financials_btn.config(text='البيانات المالية' if self.current_lang == 'ar' else 'Financials')
            if hasattr(self, 'quick_ai_btn'):
                self.quick_ai_btn.config(text='الذكاء الاصطناعي' if self.current_lang == 'ar' else 'AI')
    ...
    def _label_looks_truncated_or_weak(self, label: str) -> bool:
        txt = re.sub(r"\s+", " ", str(label or "")).strip()
        if not txt:
            return True
        if len(txt) <= 3:
            return True
        if txt.endswith(':') and len(txt) <= 6:
            return True
        alpha_count = sum(1 for ch in txt if ch.isalpha() or ("\u0600" <= ch <= "\u06ff"))
        if alpha_count <= 3:
            return True
        return False

    def _rescue_official_sec_label(self, raw_label: str, csv_row: dict, date_cols: list, layer1_active: dict) -> str:
        """
        Rescue malformed/truncated labels in official SEC statement view by
        matching the row's yearly values against Layer1 concepts. This is
        intentionally conservative: no replacement unless we get a strong match.
        """
        label = re.sub(r"\s+", " ", str(raw_label or "")).strip()
        if not self._label_looks_truncated_or_weak(label):
            return label
        ...
        year_value_map = {}
        for d in date_cols or []:
            try:
                yy = int(str(d))
            except Exception:
                continue
            nv = _num((csv_row or {}).get(d))
            if nv is not None:
                year_value_map[yy] = nv
        if not year_value_map:
            return label

        best_label = label
        best_score = 0
        for yy, concepts_map in (layer1_active or {}).items():
            if not isinstance(yy, int) or not isinstance(concepts_map, dict):
                continue
            for concept_name, concept_value in concepts_map.items():
                concept_num = _num(concept_value)
                target_num = year_value_map.get(yy)
                if concept_num is None or target_num is None:
                    continue
                if abs(concept_num - target_num) > max(1e-6, abs(target_num) * 0.0005):
                    continue

                score = 0
                concept_label = re.sub(r"\s+", " ", str(concept_name or "")).strip()
                for y2, csv_val in year_value_map.items():
                    c2 = _num(((layer1_active.get(y2, {}) or {}).get(concept_name)))
                    if c2 is None:
                        continue
                    if abs(c2 - csv_val) <= max(1e-6, abs(csv_val) * 0.0005):
                        score += 1
                if score > best_score and len(concept_label) > len(best_label):
                    best_score = score
                    best_label = concept_label

        return best_label if best_score >= 2 else label
    ...
    def _remove_selected_companies(self):
        ...
        if not sel and hasattr(self, 'company_selector_var'):
            chosen = str(self.company_selector_var.get() or '').strip()
            if chosen:
                names = list(self.companies_listbox.get(0, tk.END))
                if chosen in names:
                    sel = [names.index(chosen)]
        if not sel:
            messagebox.showinfo(self._t('msg_info'), self._translate_ui_text("لا توجد شركة محددة للحذف"))
            return
        removed = []
    ...
    def _on_company_select(self, event=None):
        ...
        if company_key in self.multi_company_data:
            ...
            if hasattr(self, 'company_selector_var'):
                self.company_selector_var.set(company_name)

            # ØªØ­Ø¯ÙŠØ« Ø¬Ù…ÙŠØ¹ Ø§Ù„Ø¹Ø±ÙˆØ¶ Ø¨Ù…Ø§ ÙÙŠÙ‡Ø§ Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø°ÙƒÙŠ
            self.display_all()
            try:
                ci = (self.current_data.get('company_info', {}) if isinstance(self.current_data, dict) else {}) or {}
    ...
    def _build_ai_analysis_tab(self):
        ...
        header = tk.Label(self.ai_analysis_tab, 
                         ...
                         bg=PALETTE['nav_surface'], fg='white', 
        ...
        desc = tk.Label(self.ai_analysis_tab,
                       ...
                       bg=PALETTE['panel'], font=FONTS['normal'], fg=PALETTE['muted'])
        ...
        # Keep references for auto-scroll/reset.
        self._ai_canvas = canvas
        self._ai_scrollbar = scrollbar
        self._ai_scrollable_frame = scrollable_frame
        ...
        def _bind_ai_mousewheel(widget):
            def _on_mousewheel(event):
                try:
        ...
        # FinancialAnalysisSystem (new institutional orchestrator output)
        fas_frame = tk.LabelFrame(
            scrollable_frame,
            text='Financial Analysis System (Orchestrator)',
            bg='#e8f4fb',
            font=FONTS['label'],
            padx=15,
            pady=10,
        )
        fas_frame.pack(fill='x', padx=10, pady=10)

        self.fas_model_label = tk.Label(
            fas_frame,
            text='Model: --',
            bg='#e8f4fb',
            font=FONTS['title'],
        ...
        self.fas_model_label.pack(anchor='w')

        self.fas_score_label = tk.Label(
            fas_frame,
            text='Score: --',
            bg='#e8f4fb',
            font=FONTS['normal'],
        ...
        self.fas_score_label.pack(anchor='w')

        self.fas_signature_label = tk.Label(
            fas_frame,
            text='Signature: --',
            bg='#e8f4fb',
            font=FONTS['normal'],
        ...
        self.fas_signature_label.pack(anchor='w')

        self.fas_integrity_label = tk.Label(
            fas_frame,
            text='Data integrity: --',
            bg='#e8f4fb',
            font=FONTS['normal'],
        ...
        self.fas_integrity_label.pack(anchor='w')

        self.fas_kpis_label = tk.Label(
            fas_frame,
            text='Primary KPIs: --',
            bg='#e8f4fb',
            font=FONTS['normal'],
        ...
        self.fas_kpis_label.pack(anchor='w')
    ...
    def _build_chat_tab(self):
        ...
        self.chat_text.tag_configure('user', foreground=PALETTE['accent'], font=FONTS['label'])
        self.chat_text.tag_configure('assistant', foreground=PALETTE['teal'], font=FONTS['label'])
        self.chat_text.tag_configure('meta', foreground=PALETTE['muted'], font=FONTS['normal'])
        self.chat_text.tag_configure('body', foreground=PALETTE['text'], font=FONTS['normal'])
    ...
    def _build_learning_tab(self):
        ...
        desc.pack(fill='x', padx=12, pady=(0, 10))

        controls = tk.Frame(self.learning_tab, bg=PALETTE['panel'])
        controls.pack(fill='x', padx=12, pady=(0, 8))
        refresh_btn = tk.Button(
            controls,
            text=self._t('learn_refresh'),
            bg=PALETTE['accent'],
            fg='white',
            font=FONTS['button'],
            relief='flat',
            command=self._refresh_learning_dashboard,
            padx=14,
            pady=6,
        )
        ...
        self.learning_status.pack(side='left', padx=12)

        body = tk.Frame(self.learning_tab, bg=PALETTE['panel'])
        body.pack(fill='both', expand=True, padx=12, pady=(0, 12))

        table_frame = tk.Frame(body, bg=PALETTE['panel'], highlightthickness=1, highlightbackground=PALETTE['border'])
        table_frame.pack(side='left', fill='both', expand=True, padx=(0, 8))
        self.learning_tree = ttk.Treeview(
            table_frame,
            columns=('category', 'count', 'samples'),
            show='headings',
            height=12,
        )
        self.learning_tree.heading('category', text=self._t('learn_category'))
        self.learning_tree.heading('count', text=self._t('learn_count'))
        self.learning_tree.heading('samples', text=self._t('learn_samples'))
        self.learning_tree.column('category', width=160, anchor='w')
        self.learning_tree.column('count', width=90, anchor='center')
        self.learning_tree.column('samples', width=520, anchor='w')
        learn_y = ttk.Scrollbar(table_frame, orient='vertical', command=self.learning_tree.yview)
        learn_x = ttk.Scrollbar(table_frame, orient='horizontal', command=self.learning_tree.xview)
        self.learning_tree.configure(yscrollcommand=learn_y.set, xscrollcommand=learn_x.set)
        table_frame.grid_columnconfigure(0, weight=1)
        table_frame.grid_rowconfigure(0, weight=1)
        self.learning_tree.grid(row=0, column=0, sticky='nsew')
        learn_y.grid(row=0, column=1, sticky='ns')
        learn_x.grid(row=1, column=0, sticky='ew')
        self.learning_tree.bind('<<TreeviewSelect>>', self._on_learning_select)

        detail_frame = tk.Frame(body, bg=PALETTE['surface_alt'], highlightthickness=1, highlightbackground=PALETTE['border'])
        detail_frame.pack(side='left', fill='y')
        detail_title = tk.Label(
            detail_frame,
            text=self._t('learn_details'),
            bg=PALETTE['surface_alt'],
            fg=PALETTE['header'],
            font=FONTS['label'],
            padx=10,
        ...
        self.learning_detail = tk.Listbox(
            detail_frame,
            width=36,
            height=14,
            bg=PALETTE['surface_alt'],
        ...
        detail_scroll = ttk.Scrollbar(detail_frame, orient='vertical', command=self.learning_detail.yview)
        self.learning_detail.configure(yscrollcommand=detail_scroll.set)
        self.learning_detail.pack(side='left', fill='both', expand=True, padx=(6, 0), pady=(0, 6))
        detail_scroll.pack(side='left', fill='y', pady=(0, 6))

        self._refresh_learning_dashboard()

    def _load_learning_mappings(self):
        path = os.path.join(BASE_DIR, "sec_learned_mappings.json")
    ...
    def _refresh_learning_dashboard(self):
        ...
        self.learning_detail.delete(0, tk.END)

        data, ts = self._load_learning_mappings()
        if not data:
            self.learning_status.configure(text=self._t('learn_file_missing'))
            return
        ...
        for cat in categories:
            ...
            if not isinstance(labels, list):
                continue
            count = len(labels)
            samples = ", ".join(labels[:5]) if labels else "—"
            self.learning_tree.insert('', 'end', values=(cat, count, samples))

        if ts:
            self.learning_status.configure(
                text=f"{self._t('learn_last_updated')}: {ts.strftime('%Y-%m-%d %H:%M')}"
            )
        else:
            self.learning_status.configure(text=self._t('learn_last_updated'))

    def _on_learning_select(self, event):
        selection = self.learning_tree.selection()
        if not selection:
            return
        item = self.learning_tree.item(selection[0])
        values = item.get('values') or []
        if not values:
            return
        category = values[0]
        data, _ = self._load_learning_mappings()
        labels = data.get(category, []) if data else []
        self.learning_detail.delete(0, tk.END)
        for label in labels:
            self.learning_detail.insert(tk.END, label)
    ...
    def _generate_auto_chat_report(self):
        if self.current_lang == 'ar':
            prompt = (
                "أنشئ تقريرًا مؤسسيًا ماليًا كاملًا ومنظمًا باللغة العربية، وليس ملخصًا قصيرًا، "
                "ويتضمن: 1) ملخصًا تنفيذيًا، 2) تحليلًا تاريخيًا، 3) نموذج العمل، 4) مزايا تنافسية، "
                "5) جودة الأرباح، 6) سلامة البيانات، 7) تقييمًا نسبيًا للأقران، 8) التوقعات المالية، "
                "9) المخاطر، 10) التقييم، 11) السيناريوهات (Base/Bull/Bear)، 12) الحكم النهائي، "
                "13) توصية نهائية، 14) ملاحظات منهجية. "
                "مهم: لا تخترع أي قيمة غير متاحة، واستخدم دائمًا الوسم DATA_NOT_AVAILABLE_FROM_AUTHORITATIVE_SOURCES "
                "عند غياب المصدر الموثوق."
            )
        elif self.current_lang == 'fr':
            prompt = (
    ...
    def _export_chat_word_report(self):
        ...
        if not excel_path:
            excel_path = filedialog.askopenfilename(
                title=self._translate_ui_text("اختر ملف Excel المصدري للشركة"),
        ...
        if excel_path:
            try:
                ...
                messagebox.showwarning(
                    self._t('msg_warning'),
                    f"{self._translate_ui_text('فشل تحليل ملف Excel المصدر. تأكد من سلامة الملف والمحتوى.')}\n{e}"
                )
        ...
        if not fn:
            messagebox.showinfo(
                ...
                self._translate_ui_text("لم يتم حفظ التقرير Word لأن المستخدم ألغى العملية.")
            )
            return
        ...
        def _export_from_strict_json(payload):
            ...
            def _append_excel_sheet_tables():
                ...
                if not isinstance(frames, dict) or not frames:
                    return
                doc.add_heading('19. ??????? ???????? ??????? ?? ??? Excel', level=2)
                doc.add_paragraph(
                    '??? ????? ???? ?????? ??????? ??? ?? ?? ??? ?????? ?????? ????? ??????? ???????.'
                )
                for sh_name, sh_df in frames.items():
                    try:
                        ...
                        doc.add_heading(f"????: {sh_name}", level=3)
            ...
            def _append_source_excel_trace():
                if not context.get('_source_excel_path'):
                    return
                _add_section('18. ???? ??? Excel ??????')
                _add_labeled('??? ??????', context.get('_source_excel_path'))
                ...
                for sec in sections:
                    ...
                    for block in (sec.get('blocks') or []):
                        ...
                        if btype == 'text':
                            ...
                            if not _md_table_to_doc(text, f"???? {title_text}"):
            ...
            meta.rows[3].cells[1].text = str(rating.get('label', '??? ????'))
            meta.rows[4].cells[0].text = '?????? / ????? / ?????'
            meta.rows[4].cells[1].text = f"{rating.get('score_100', '??? ????')}/100 | {rating.get('confidence', '??? ????')}% | {rating.get('time_horizon', '??? ????')}"
            doc.add_paragraph(
                "???????? ?????????: ??? ??????? ????? ??? ???? ?????? ?????? ?????. "
                "????? ??? ??? ??????? ???? ?? ????? ?? ??? ???? fundamentals ??????? ???? ??????? ??????."
            )
            ...
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(hf.get('analysis') or '??? ????'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(hf.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "?????? ???????? ?? ????? ?????? ????? ??????? ?? ????? ??????? ??? ????? ???????? ??????. "
                "????? ????? ???? ????? ????? ????????? ?? ???? ?? ???? ???????? ?????? ????? ????? ????? ???????? ??? ?????. "
                    "??? ??? ????? ????????? ??? ??? ????/????? ??????? ???? ????? ??? ???? ????? ??????."
            ...
            def _append_full_timeseries_tables():
                ...
                for lbl, ck in core_keys:
                    ...
                    for i, yy in enumerate(years, start=1):
                        ...
                        vals.append(v)
                        rr[i].text = "—" if v is None else f"{v:,.6g}"
            ...
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(pa.get('analysis') or '??? ????'))
            _add_labeled('???? ?????????', pa.get('sustainability_view'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(pa.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "??????? ?????? ?????? ????? ??? ??????? ????????? ???????? ???????? ????? ??? ?????. "
                "?????? ROE ???? ?? ???? ?? ??? ??????? ??????? ???? ??????? ?? ????? ???????. "
                "?????? ???????? ???? ????? ?????? ?????? ROIC ?? ????? ???????? ?? ??? ??? ???? ?????? ??? ??? ???? ?????? ?? ?? ???? ??????."
            ...
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(ls.get('analysis') or '??? ????'))
            _add_labeled('??? ?????????', ls.get('balance_sheet_view'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(ls.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "??? ??????? ???????? ?? ???? ???????? ????? ????? ??? ???? ?????? ?????? ????? ????? ?????? ??? ???? ?????????? ???????. "
                "??? ??? ?????? ????? ????? ?? ????? ??????? ??????? ???????? ????? ????? ?? ???? ??????? ??? ???? ??????? ?????? ??? ???????."
            )
            ...
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(cf.get('analysis') or '??? ????'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(cf.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "???? ??????? ?? ??? ?????? ?? ????? ???????: ??????? ???? ?? ????? ??? ??? ?????? ???? ??? ??????? ??????????. "
                "?????? ?????? ???? ???????? ???? ??????? ????? ?? ????? ?????? ?????????? ?????????? ??? ??? ??????? "
                "????? ??? ?????? ?????? ???? ?????? ????? ????? ???? ??? ?? ????? ??????? ????."
            ...
            _add_section('7. ????? ????? ??? ????? ???? ?????? ??????????')
            _add_labeled('?????? ????? ??? ?????', ce.get('capital_efficiency_review'))
            _add_labeled('??? ?????? ??????????', ce.get('economic_value_creation'))
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(ce.get('analysis') or '??? ????'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(ce.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "??? ????? ???? ?? ???? ?????: ?? ????? ???? ???? ?? ????????? "
                "????? ?????? ROIC ????? ??? ????? ???? ??????? ????? ?????? ??? ??? ????? ???? ?????? ????? ?????. "
                "??? ??? ????? ?????? ?? ????? ??? ?????? ?????? ???? ??? ???? ??? ?? ??? ????? ??????."
            ...
            _add_section('8. ??????? ??????? ?????????? ????????')
            _add_labeled('???? ????????', sp.get('comparison_basis'))
            _add_labeled('??????? ??????', sp.get('relative_positioning'))
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(sp.get('analysis') or '??? ????'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(sp.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "???????? ???????? ???? ??????? ??????? ???????? ??????? ????? ?? ???? ?? ???? ?????? ?? ???? ???. "
                "??????? ?????? ??? ?? ???? ??? ????? ???????? ??????? ??? ??? ????? ???????? ????????? ?????? ?? ??? ???? ?????? ???? ??? ????? ?????? ??????."
            )
            ...
            for sec_name, sec_key in [
                ...
                for x in (rk.get(sec_key) or []):
                    doc.add_paragraph(f"• {x}")
            _add_labeled('?????? ????????? ???????', rk.get('overall_risk_view'))
            _add_expanded_comment(
                "??????? ??????? ??????? ?? ????? ??????????? ???????? ?? ????? ??? ????? ????? ????????. "
                "??????? ????????? ?????? ????? ????? ??? ?????? ?????????? ????? ??????? ????????? ?????? ???????? ??? ???? ???????. "
                "????? ????? ??????? ???? ?????? ?????? ??????? ?????? ?? ?????? ????? ??????? ????????."
            ...
            _add_section('10. ????? ???????? ??????????')
            _add_labeled('????????? ???????', fc.get('base_case'))
            _add_labeled('????????? ????????', fc.get('bull_case'))
            _add_labeled('????????? ???????', fc.get('bear_case'))
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(fc.get('scenario_interpretation') or '??? ????'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(fc.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "????????? ??????? ??? ?????? ??????? ?? ????? ??????. "
                "???? ???? ?????? ??? ????????????? ?????? ???? ????? ??????? ????? ???? ??????. "
                "??? ???? ?????? ???????? ????? ?? ??? ??????? ??? ????? ????? ???? ??? ??????? ???????? ??? ??? ?????."
            ...
            _add_section('11. ????? ???????')
            _add_labeled('???? ???????', va.get('valuation_snapshot'))
            _add_labeled('????? ??????? ???????', va.get('fairness_assessment'))
            doc.add_paragraph('??????? ?????????:')
            doc.add_paragraph(str(va.get('multiple_interpretation') or '??? ????'))
            doc.add_paragraph('?? ???? ????? ??? ?????????')
            doc.add_paragraph(str(va.get('investor_implication') or '??? ????'))
            _add_expanded_comment(
                "?????? ?????? ???: ?? ?????? ??????? ??: ?? ????? ?????? ???? ?????? ?????? ???????? ???????? "
                "??? ??????? ????? ?????? ?? ???? ????????? ?????? ??? ??????? ??????? "
                "????? ???? ????? ??? ???? ???? ???? ????? ??? ???? ????????? ???? ???? ????????."
            ...
            _add_section('12. ????? ?????????? ???????')
            _add_labeled('???????', final_case.get('rating_label', rating.get('label', '??? ????')))
            _add_labeled('?????? ?? 100', final_case.get('score_100', rating.get('score_100', '??? ????')))
            _add_labeled('?????', f"{final_case.get('confidence', rating.get('confidence', '??? ????'))}%")
            doc.add_paragraph('??? 5 ??????:')
            ...
            for x in (final_case.get('top_5_monitoring_points') or []):
                doc.add_paragraph(f"• {x}")
            doc.add_paragraph('??????? ???? ?? ????? ???????:')
            doc.add_paragraph(str(final_case.get('what_could_change_the_rating') or '??? ????'))
            _add_expanded_comment(
                "??????? ??????? ??? ?? ???? ????? ?????? ????????? ?? ?? ???? ?????. "
                "?? ????? ??????? ???? ?? ???? ???? ?????? ??????? ?????? ???????? ????? ???????? "
                "????? ??????? ?????? ????? ?????????. ???? ?????? ??? ???????? ???? ????? ??????."
            ...
            _add_section('13. ??????? ????????? ????????', payload.get('closing_summary'))
            _add_expanded_comment(
                "??????? ???????? ???? ??? ??????: ???? ?????? ????? ?????. "
                "???? ???? ???????? ???? ????? ?????? ??? ?????? ?? ????? ???? ????? ???? ?????? "
                "??? ??? ?????? ??? ???????? ????????? ???? ?????? ??????? ?????? ??????."
            ...
            _add_section('14. ???????? ???????')
            _add_labeled('????? ??????', f"{sr.get('numerical_depth', '??? ????')}/10")
            _add_labeled('?????? ???????', f"{sr.get('narrative_quality', '??? ????')}/10")
            _add_labeled('?????? ?????????', f"{sr.get('economic_reasoning', '??? ????')}/10")
            _add_labeled('???? ???????', f"{sr.get('risk_analysis_quality', '??? ????')}/10")
            _add_labeled('???? ???????', f"{sr.get('valuation_quality', '??? ????')}/10")
            _add_labeled('????? ???????', f"{sr.get('consistency_of_final_rating', '??? ????')}/10")
            _append_full_timeseries_tables()
        ...
        def _assessment_text(metric, value):
            ...
            if raw == '—':
                return L('??? ????', 'Unavailable', 'Indisponible')
            if raw == 'Strong':
                return L('???', 'Strong', 'Fort')
            if raw == 'Moderate':
                return L('?????', 'Moderate', 'Moyen')
            if raw == 'Weak':
                return L('????', 'Weak', 'Faible')
            return L('?????', 'Neutral', 'Neutre')
        ...
        # 0) Cover
        doc.add_heading(L('??????? ?????? ??????', 'Institutional Financial Analysis Report', 'Rapport financier institutionnel'), level=1)
        doc.add_paragraph(L(f'??????: {name} ({ticker})', f'Company: {name} ({ticker})', f'Société: {name} ({ticker})'))
        doc.add_paragraph(L(f'???????: {datetime.now().strftime("%Y-%m-%d")}', f'Date: {datetime.now().strftime("%Y-%m-%d")}', f'Date: {datetime.now().strftime("%Y-%m-%d")}'))
        doc.add_paragraph(L('?????? ???????: SEC EDGAR (XBRL)', 'Primary Source: SEC EDGAR (XBRL)', 'Source principale : SEC EDGAR (XBRL)'))
        doc.add_paragraph(L('??????: ????? ?????? + ????? ????? + ?????????? ?????? + ?????? ??????? ??????',
        ...
        # 1) Executive summary
        doc.add_heading(L('1. ?????? ????????', '1. Executive Summary', '1. Résumé exécutif'), level=2)
        ...
        doc.add_paragraph(L(f'???? ?????? ??????: {_fmt_num(qscore, 1)}/100',
                            f'Overall quality score: {_fmt_num(qscore, 1)}/100',
                            f'Score global de qualité: {_fmt_num(qscore, 1)}/100'))
        doc.add_paragraph(L(f'???? ???? ??????: {_fmt_pct(gm)} | ???? ????: {_fmt_pct(nm)} | ?????? ??? ???? ??????? ROE: {_fmt_pct(roe)}',
        ...
        doc.add_paragraph(L(f'??????? ??????? (?????? ???????): {_fmt_num(cr, 2)} | ??????? (?????/???? ???????): {_fmt_num(dte, 2)}',
        ...
        if _n(nm) is not None and _n(nm) >= 0.20:
            strengths.append(L(f"????? ????? ???? ({_fmt_pct(nm)})", f"Strong net profitability ({_fmt_pct(nm)})", f"Rentabilité nette solide ({_fmt_pct(nm)})"))
        if _n(roe) is not None and _n(roe) >= 0.20:
            strengths.append(L(f"???? ????? ??? ???? ??????? ({_fmt_pct(roe)})", f"High return on equity ({_fmt_pct(roe)})", f"ROE élevé ({_fmt_pct(roe)})"))
        if _n(gm) is not None and _n(gm) >= 0.35:
            strengths.append(L(f"???? ?????? ????? ({_fmt_pct(gm)})", f"High gross margin ({_fmt_pct(gm)})", f"Marge brute élevée ({_fmt_pct(gm)})"))
        if _n(cr) is not None and _n(cr) < 1.0:
            cautions.append(L(f"??????? ????? ????? ??? 1 ({_fmt_num(cr,2)})", f"Short-term liquidity below 1 ({_fmt_num(cr,2)})", f"Liquidité court terme < 1 ({_fmt_num(cr,2)})"))
        if _n(dte) is not None and _n(dte) > 1.5:
            cautions.append(L(f"????? ????? ?????? ?????? ({_fmt_num(dte,2)})", f"Relatively elevated leverage ({_fmt_num(dte,2)})", f"Levier relativement élevé ({_fmt_num(dte,2)})"))
        if not strengths:
            strengths.append(L("???? ??????? ?????? ?? ??????? ????? ??? ???????.", "Acceptable operating quality with room for improvement.", "Qualité opérationnelle acceptable avec marge d’amélioration."))
        if not cautions:
            cautions.append(L("?? ???? ?????? ???? ????? ?? ???????? ????????.", "No immediate critical signal in core metrics.", "Aucun signal critique immédiat dans les indicateurs clés."))
        doc.add_paragraph(L('???? ????? ????????:', 'Key strengths:', 'Forces principales :'))
        for s in strengths[:5]:
            doc.add_paragraph(f"• {s}")
        doc.add_paragraph(L('???? ????? ??????:', 'Watch items:', 'Points de vigilance :'))
        for c in cautions[:5]:
            doc.add_paragraph(f"• {c}")

        # 2) Historical financial statements snapshot
        doc.add_heading(L('2. ???????? ??????? ????????? (????? ?????)', '2. Historical Financial Data (Billions USD)', '2. Données financières historiques (Md USD)'), level=2)
        t_hist = doc.add_table(rows=1, cols=5)
        t_hist.style = 'Table Grid'
        h = t_hist.rows[0].cells
        h[0].text = L('?????', 'Metric', 'Indicateur')
        h[1].text = str(years_focus[0] if len(years_focus) > 0 else '—')
        h[2].text = str(years_focus[1] if len(years_focus) > 1 else '—')
        h[3].text = str(years_focus[2] if len(years_focus) > 2 else '—')
        h[4].text = L('???????', 'Trend', 'Tendance')

        hist_rows = [
            (L('?????????', 'Revenue', 'Revenu'), ['Revenues', 'Revenue', 'SalesRevenueNet', 'RevenueFromContractWithCustomerExcludingAssessedTax']),
            (L('?????? ?????', 'Gross Profit', 'Marge brute'), ['GrossProfit']),
            (L('????? ????????', 'Operating Income', 'Résultat opérationnel'), ['OperatingIncomeLoss', 'IncomeLossFromOperations']),
            (L('???? ?????', 'Net Income', 'Résultat net'), ['NetIncomeLoss', 'ProfitLoss']),
            (L('?????? ??????', 'Total Assets', 'Total Actifs'), ['Assets', 'TotalAssets']),
            (L('?????? ??????????', 'Total Liabilities', 'Total Passifs'), ['Liabilities', 'TotalLiabilities']),
            (L('???? ???????', 'Shareholders Equity', 'Capitaux propres'), ['StockholdersEquity', 'StockholdersEquityIncludingPortionAttributableToNoncontrollingInterest', 'TotalEquity']),
            (L('?????? ?????? ????????', 'Operating Cash Flow', 'Flux de trésorerie opérationnel'), ['NetCashProvidedByUsedInOperatingActivities', 'OperatingCashFlow']),
            (L('??????? ?????????', 'Capital Expenditures', 'Dépenses d’investissement'), ['PaymentsToAcquirePropertyPlantAndEquipment', 'CapitalExpenditures']),
        ]
        ...
        for label, keys in hist_rows:
            ...
            r[0].text = label
            r[1].text = _fmt_b_from_million(vals[0]) if len(vals) > 0 else '—'
            r[2].text = _fmt_b_from_million(vals[1]) if len(vals) > 1 else '—'
            r[3].text = _fmt_b_from_million(vals[2]) if len(vals) > 2 else '—'
            r[4].text = trend

        # 3) Ratios
        doc.add_heading(L('3. ????? ????? ???????', '3. Financial Ratio Analysis', '3. Analyse des ratios financiers'), level=2)
        doc.add_heading(L('3.1 ??? ???????', '3.1 Profitability', '3.1 Rentabilité'), level=3)
        t_profit = doc.add_table(rows=1, cols=5)
        t_profit.style = 'Table Grid'
        hp = t_profit.rows[0].cells
        hp[0].text = L('??????', 'Ratio', 'Ratio')
        hp[1].text = str(years_focus[0] if len(years_focus) > 0 else '—')
        hp[2].text = str(years_focus[1] if len(years_focus) > 1 else '—')
        hp[3].text = str(years_focus[2] if len(years_focus) > 2 else '—')
        hp[4].text = L('???????', 'Assessment', 'Évaluation')

        profit_items = [
            (L('???? ????? ????????', 'Gross Margin', 'Marge brute'), 'gross_margin'),
            (L('???? ????? ????????', 'Operating Margin', 'Marge opérationnelle'), 'operating_margin'),
            (L('???? ???? ?????', 'Net Margin', 'Marge nette'), 'net_margin'),
            ('ROA', 'roa'),
            ('ROE', 'roe'),
            ('ROIC', 'roic'),
        ]
        for label, key in profit_items:
            ...
            r[0].text = label
            r[1].text = _fmt_pct(vals[0]) if len(vals) > 0 else '—'
            r[2].text = _fmt_pct(vals[1]) if len(vals) > 1 else '—'
            r[3].text = _fmt_pct(vals[2]) if len(vals) > 2 else '—'
        ...
        doc.add_heading(L('3.2 ??????? ????????', '3.2 Liquidity and Leverage', '3.2 Liquidité et levier'), level=3)
        t_liq = doc.add_table(rows=1, cols=5)
        t_liq.style = 'Table Grid'
        hl = t_liq.rows[0].cells
        hl[0].text = L('??????', 'Ratio', 'Ratio')
        hl[1].text = str(years_focus[0] if len(years_focus) > 0 else '—')
        hl[2].text = str(years_focus[1] if len(years_focus) > 1 else '—')
        hl[3].text = str(years_focus[2] if len(years_focus) > 2 else '—')
        hl[4].text = L('???????', 'Assessment', 'Évaluation')

        liq_items = [
            (L('?????? ???????', 'Current Ratio', 'Ratio courant'), 'current_ratio', False),
            (L('?????? ???????', 'Quick Ratio', 'Quick ratio'), 'quick_ratio', False),
            (L('?????? ???????', 'Cash Ratio', 'Ratio de trésorerie'), 'cash_ratio', False),
            (L('????? ??? ???? ???????', 'Debt to Equity', 'Dette / Capitaux propres'), 'debt_to_equity', False),
            (L('????? ???????', 'Interest Coverage', 'Couverture des intérêts'), 'interest_coverage', False),
            (L('???? ?????? ?????? ????', 'FCF Yield', 'Rendement FCF'), 'fcf_yield', True),
        ]
        for label, key, is_pct in liq_items:
            ...
            if is_pct:
                ...
                r[1].text = _fmt_num(vals[0], 2) if len(vals) > 0 else '—'
                r[2].text = _fmt_num(vals[1], 2) if len(vals) > 1 else '—'
                r[3].text = _fmt_num(vals[2], 2) if len(vals) > 2 else '—'
            r[4].text = _assessment_text(key, vals[-1] if vals else None)
        doc.add_paragraph(L('??????? ?????????: ??? ??????? ?? ???? ?????? ???????? ??? ???? ???????? ??????? ????????? ???? ??????.',
        ...
        doc.add_heading(L('4. ?????? ??????? ???????????', '4. Strategic Analysis Indicators', '4. Indicateurs stratégiques'), level=2)
        t_str = doc.add_table(rows=1, cols=4)
        t_str.style = 'Table Grid'
        hs = t_str.rows[0].cells
        hs[0].text = L('??????', 'Indicator', 'Indicateur')
        hs[1].text = str(years_focus[0] if len(years_focus) > 0 else '—')
        hs[2].text = str(years_focus[1] if len(years_focus) > 1 else '—')
        hs[3].text = str(years_focus[2] if len(years_focus) > 2 else '—')

        strat_items = [
            (L('???? ???? ?????????', 'Investment Score', 'Score d’investissement'), ['investment_score', 'Investment_Score', 'investment_quality_score']),
            (L('?????? ?????????', 'Economic Spread', 'Spread économique'), ['economic_spread', 'Economic_Spread']),
            (L('?????? ???????', 'Fair Value Estimate', 'Juste valeur'), ['fair_value_estimate', 'Fair_Value_Estimate', 'Fair_Value', 'fair_value']),
            (L('???? Altman Z', 'Altman Z Score', 'Score Altman Z'), ['altman_z_score', 'Altman_Z_Score']),
            (L('???? Piotroski F', 'Piotroski F Score', 'Score Piotroski F'), ['piotroski_f_score', 'Piotroski_F_Score']),
        ]
        for label, keys in strat_items:
            vals = []
            for y in years_focus[:3]:
                sv = _pick_any(y, keys)
                if sv is None and label == L('?????? ?????????', 'Economic Spread', 'Spread économique'):
            ...
            r[0].text = label
            if label == L('?????? ???????', 'Fair Value Estimate', 'Juste valeur'):
                # Fair value is typically per share; keep it numeric (not B$).
                r[1].text = _fmt_num(vals[0], 2) if len(vals) > 0 else '—'
                r[2].text = _fmt_num(vals[1], 2) if len(vals) > 1 else '—'
                r[3].text = _fmt_num(vals[2], 2) if len(vals) > 2 else '—'
            elif label in {L('?????? ?????????', 'Economic Spread', 'Spread économique')}:
                r[1].text = _fmt_pct(vals[0]) if len(vals) > 0 else '—'
                r[2].text = _fmt_pct(vals[1]) if len(vals) > 1 else '—'
                r[3].text = _fmt_pct(vals[2]) if len(vals) > 2 else '—'
            elif label in {L('???? ???? ?????????', 'Investment Score', 'Score d’investissement')}:
                r[1].text = f"{_fmt_num(vals[0], 1)}/100" if len(vals) > 0 and _n(vals[0]) is not None else '—'
                r[2].text = f"{_fmt_num(vals[1], 1)}/100" if len(vals) > 1 and _n(vals[1]) is not None else '—'
                r[3].text = f"{_fmt_num(vals[2], 1)}/100" if len(vals) > 2 and _n(vals[2]) is not None else '—'
            else:
                r[1].text = _fmt_num(vals[0], 2) if len(vals) > 0 else '—'
                r[2].text = _fmt_num(vals[1], 2) if len(vals) > 1 else '—'
                r[3].text = _fmt_num(vals[2], 2) if len(vals) > 2 else '—'

        doc.add_paragraph(L('????? ?????: ???? ??? ????? ?????? ?? ???? ?????????? ??????? ????????? (ROIC - WACC)? ?????? ??????? ???????.',
        ...
        # 5) Forecast block
        doc.add_heading(L('5. ?????????? ?????? (2026-2035)', '5. Forecast Scenarios (2026-2035)', '5. Scénarios de prévision (2026-2035)'), level=2)
        t_fc = doc.add_table(rows=1, cols=6)
        t_fc.style = 'Table Grid'
        hf = t_fc.rows[0].cells
        hf[0].text = L('?????', 'Year', 'Année')
        hf[1].text = L('????????? (????????? ???????)', 'Revenue Base', 'Revenu Base')
        hf[2].text = L('????????? (????????? ????????)', 'Revenue Bull', 'Revenu Bull')
        hf[3].text = L('????????? (????????? ???????)', 'Revenue Bear', 'Revenu Bear')
        hf[4].text = L('???? ????? (????????? ???????)', 'Net Income Base', 'Résultat net Base')
        hf[5].text = L('?????', 'Confidence', 'Confiance')
        ...
        if forecast_df is not None and not forecast_df.empty:
            for _, fr in forecast_df.head(10).iterrows():
                ...
                r[5].text = '—' if confv is None else f"{confv:.1f}%"
        else:
            r = t_fc.add_row().cells
            r[0].text = '—'
            r[1].text = '—'
        ...
        doc.add_heading(L('5.1 ???????? ???????? ?????????', '5.1 Visual Forecast and Trend Charts', '5.1 Graphiques explicatifs'), level=3)
        ...
        if valid_hist:
            ...
            p_hist = _make_chart(
                L('????? ????????? ????? ????? (B$)', 'Revenue and Net Income Trend (B$)', 'Tendance revenu et résultat net (Md$)'),
                xh,
                {
                    L('?????????', 'Revenue', 'Revenu'): yr,
                    L('???? ?????', 'Net Income', 'Résultat net'): yn,
                },
                L('????? ?????', 'Billion USD', 'Md USD'),
            )
            if p_hist:
                doc.add_picture(p_hist, width=Inches(6.6))
                if len(yr) >= 2 and yr[0] not in (None, 0):
                    growth_hist = ((yr[-1] - yr[0]) / abs(yr[0])) * 100.0
                    doc.add_paragraph(L(f'????? ?????: ????????? ????? ????? {growth_hist:+.1f}% ??? ?????? ???????? ?? ????? ???? ?????.',
        ...
        if valid_m:
            ...
            p_margin = _make_chart(
                L('???? ????? ??????? (%)', 'Profitability Margin Trend (%)', 'Évolution des marges (%)'),
                xm,
                {
                    L('???? ??????', 'Gross Margin', 'Marge brute'): ygm,
                    L('???? ??????', 'Operating Margin', 'Marge opérationnelle'): yom,
                    L('???? ????', 'Net Margin', 'Marge nette'): ynm,
                },
        ...
        if forecast_df is not None and not forecast_df.empty:
            ...
            if valid_f:
                ...
                p_fc = _make_chart(
                    L('?????? ??????: Bear/Base/Bull (????????? B$)', 'Forecast Paths: Bear/Base/Bull (Revenue B$)', 'Trajectoires prévisionnelles : Bear/Base/Bull (Revenu Md$)'),
                    xf,
                    {
                        'Base': yb,
                        'Bull': ybu,
        ...
        # 6) AI analysis
        doc.add_heading(L('6. ???? ??????? ?????', '6. AI Analysis Snapshot', '6. Synthèse IA'), level=2)
        ...
        doc.add_paragraph(L(f"?????? ????????: {'??? ????' if fp is None else f'{fp * 100:.2f}%'}",
                            f"Fraud probability: {'Unavailable' if fp is None else f'{fp * 100:.2f}%'}",
                            f"Probabilité de fraude: {'Indisponible' if fp is None else f'{fp * 100:.2f}%'}"))
        doc.add_paragraph(L(f"?????? ?????? (3 ?????): {'??? ????' if fail3 is None else f'{fail3 * 100:.2f}%'}",
                            f"Failure probability (3Y): {'Unavailable' if fail3 is None else f'{fail3 * 100:.2f}%'}",
                            f"Probabilité de défaillance (3 ans): {'Indisponible' if fail3 is None else f'{fail3 * 100:.2f}%'}"))
        doc.add_paragraph(L(f"?????? ?????? (5 ?????): {'??? ????' if fail5 is None else f'{fail5 * 100:.2f}%'}",
                            f"Failure probability (5Y): {'Unavailable' if fail5 is None else f'{fail5 * 100:.2f}%'}",
                            f"Probabilité de défaillance (5 ans): {'Indisponible' if fail5 is None else f'{fail5 * 100:.2f}%'}"))
        doc.add_paragraph(L(f"???? ???? ?????????: {_fmt_num(qscore, 1)}/100",
        ...
        if ai_chart_path and Path(ai_chart_path).exists():
            doc.add_heading(L('6.1 ???? ??????? ???????', '6.1 AI Risk and Quality Chart', '6.1 Graphe risque et qualité IA'), level=3)
            ...
            risk_text = L(
                f"????? ????? ???????: ???????? {_risk_level_text(fp_pct)} ({'??? ????' if fp_pct is None else f'{fp_pct:.1f}%'}), "
                f"?????? ???? 3 ????? {_risk_level_text(f3_pct)} ({'??? ????' if f3_pct is None else f'{f3_pct:.1f}%'}), "
                f"????? ??????? {_risk_level_text(wc_pct)} ({'??? ????' if wc_pct is None else f'{wc_pct:.1f}%'}).",
                f"Heatmap interpretation: fraud {_risk_level_text(fp_pct)} ({'Unavailable' if fp_pct is None else f'{fp_pct:.1f}%'}), "
                f"3Y failure {_risk_level_text(f3_pct)} ({'Unavailable' if f3_pct is None else f'{f3_pct:.1f}%'}), "
                f"and liquidity crisis {_risk_level_text(wc_pct)} ({'Unavailable' if wc_pct is None else f'{wc_pct:.1f}%'}).",
                f"Interprétation heatmap : fraude {_risk_level_text(fp_pct)} ({'Indisponible' if fp_pct is None else f'{fp_pct:.1f}%'}), "
                f"défaillance 3 ans {_risk_level_text(f3_pct)} ({'Indisponible' if f3_pct is None else f'{f3_pct:.1f}%'}), "
                f"et crise de liquidité {_risk_level_text(wc_pct)} ({'Indisponible' if wc_pct is None else f'{wc_pct:.1f}%'})."
            )
            radar_text = L(
                f"????? ????? ??????: ROIC={roic_n:.1f}/100? Spread={spread_n:.1f}/100? FCF Yield={fcf_n:.1f}/100? "
                f"Z-Score={z_n:.1f}/100? Investment Score={inv_n:.1f}/100. ??????? ???? "
                f"{'???? ????' if inv_n >= 75 else ('???? ??????' if inv_n >= 55 else '???? ?????')} ??????? ?????? ??????? ????????.",
        ...
        doc.add_heading(L('7. ????? ?????????? ???????', '7. Final Investment Verdict', '7. Verdict d’investissement final'), level=2)
        t_v = doc.add_table(rows=1, cols=5)
        t_v.style = 'Table Grid'
        hv = t_v.rows[0].cells
        hv[0].text = L('?????', 'Year', 'Année')
        hv[1].text = L('?????', 'Verdict', 'Verdict')
        hv[2].text = L('?????', 'Confidence', 'Confiance')
        hv[3].text = 'P/E'
        hv[4].text = 'P/B'
        ...
        for yy in years_focus[:3]:
            ...
            if _n(nm_y) is not None and _n(roe_y) is not None and _n(nm_y) > 0.10 and _n(roe_y) > 0.12:
                ...
                verdict = L('??????', 'WATCH', 'SURVEILLER')
            else:
                verdict = L('??????', 'REVIEW', 'REVIEW')
        ...
        doc.add_heading(L('8. ????? ???????? ????????? ???? ???????', '8. Accounting Integrity and Audit Trail', '8. Intégrité comptable et piste d’audit'), level=2)
        t_a = doc.add_table(rows=1, cols=5)
        t_a.style = 'Table Grid'
        ha = t_a.rows[0].cells
        ha[0].text = L('?????', 'Year', 'Année')
        ha[1].text = L('?????? (B$)', 'Assets (B$)', 'Actifs (Md$)')
        ha[2].text = L('?????????? (B$)', 'Liabilities (B$)', 'Passifs (Md$)')
        ha[3].text = L('???? ??????? (B$)', 'Equity (B$)', 'Capitaux propres (Md$)')
        ha[4].text = L('??? ???????', 'Balance Check', 'Contrôle bilan')
        ...
        for yy in years_bal:
            ...
            if _n(assets) is not None and _n(liabilities) is not None and _n(equity) is not None and _n(assets) != 0:
                ...
                status = L('?????', 'PASS', 'PASS') if diff <= 0.08 else L('????', 'CHECK', 'CHECK')
                if diff <= 0.08:
        ...
        t_q.rows[0].cells[0].text = L('???? ??????', 'Quality Dimension', 'Dimension qualité')
        t_q.rows[0].cells[1].text = L('???????', 'Rating', 'Note')
        ...
        quality_rows = [
            (L('???????', 'Profitability', 'Rentabilité'), _stars(((_n(gross_latest) or 0.0) * 100.0) + 35.0)),
            (L('???????', 'Liquidity', 'Liquidité'), _stars(((_n(current_latest) or 0.0) * 40.0) + 30.0)),
            (L('???????', 'Leverage', 'Levier'), _stars(100.0 - min(100.0, ((_n(debt_latest) or 0.0) * 35.0)))),
            (L('????? ??? ?????', 'Capital Efficiency', 'Efficacité du capital'), _stars(((_n(roic_latest) or 0.0) * 100.0) + 30.0)),
            (L('???? ?????????', 'Investment Quality', 'Qualité d’investissement'), _stars(qscore)),
        ]
        ...
        if final_index >= 85:
            final_rec = L('???? ???', 'Strong Buy', 'Achat fort')
        elif final_index >= 70:
            final_rec = L('????', 'Buy', 'Achat')
        elif final_index >= 55:
            final_rec = L('??????', 'Hold', 'Conserver')
        elif final_index >= 40:
            final_rec = L('??????', 'Watch', 'Surveiller')
        else:
            final_rec = L('????', 'Avoid', 'Éviter')

        final_conf = max(55.0, min(98.5, (qscore * 0.55) + (100.0 - min(100.0, risk_penalty)) * 0.35 + (balance_pass_count * 2.0)))
        doc.add_heading(L('8.1 ??????? ???????? ????????', '8.1 Final Synthesis and Recommendation', '8.1 Synthèse et recommandation finale'), level=3)
        doc.add_paragraph(L(f'???? ????? ??????????: {final_index:.1f}/100',
                            f'Investment verdict index: {final_index:.1f}/100',
                            f'Indice de verdict: {final_index:.1f}/100'))
        doc.add_paragraph(L(f'??????? ????????: {final_rec}',
                            f'Final recommendation: {final_rec}',
                            f'Recommandation finale: {final_rec}'))
        doc.add_paragraph(L(f'???? ?????: {final_conf:.1f}%',
        ...
        doc.add_paragraph(L('??? ??????? SEC ?????????:', 'Recent SEC filings used:', 'Derniers dépôts SEC utilisés :'))
        if filings:
            for f in filings[:8]:
                form = f.get('form') or '—'
    ...
    def _clear_companies(self):
        if messagebox.askyesno(self._t('msg_confirm'), self._translate_ui_text("هل تريد مسح كل الشركات من القائمة؟")):
    ...
    def fetch_data(self):
        ...
        if not companies:
            messagebox.showerror(self._t('msg_error'), self._translate_ui_text("لا توجد شركات للتحميل"))
            return
        try:
            start_year = int(self.start_year_var.get())
            end_year = int(self.end_year_var.get())
        except:
            messagebox.showerror(self._t('msg_error'), self._translate_ui_text("المدى الزمني غير صالح"))
            return
        if start_year > end_year:
            messagebox.showerror(self._t('msg_error'), self._translate_ui_text("يجب أن تكون سنة البداية أقل من أو تساوي سنة النهاية"))
            return
        ...
        if self.current_lang == 'ar':
            self.progress_label.config(text=f"بدء تحميل البيانات تلقائياً | نوع التقرير: {filing_type} | الفترة: {start_year}-{end_year}")
        elif self.current_lang == 'fr':
        ...
        def worker():
            try:
                for idx, comp in enumerate(companies, start=1):
                    cb(self._translate_ui_text(f"({idx}/{len(companies)}) جارٍ جلب بيانات: {comp} ..."))
                    res = self.fetcher.fetch_company_data(comp, start_year, end_year, filing_type, callback=cb, include_all_concepts=False)
                    if res.get('success'):
                        ...
                        cb(self._translate_ui_text(f"فشل جلب {comp}: {res.get('error')}"))
                self.root.after(0, self.display_all)
                self.root.after(0, self._refresh_company_selector)
            finally:
    ...
    def display_ai_analysis(self):
        ...
        if not self.current_data:
            ...
            self.fas_model_label.config(text='النموذج: --')
            self.fas_score_label.config(text='الدرجة: --')
            self.fas_signature_label.config(text='البصمة: --')
            self.fas_integrity_label.config(text='سلامة البيانات: --')
            self.fas_kpis_label.config(text='أهم مؤشرات الأداء: --')
            self._clear_embedded_chart('_ai_chart_canvas')
            self._reset_ai_scroll()
            return
        ...
        try:
            ...
            is_bank_like = (
                sub_sector_l in bank_like_profiles
                or sub_sector_l.endswith('_bank')
            )
            ...
            if isinstance(rev_conf_avg, (int, float)) and isinstance(ni_conf_avg, (int, float)):
                ...
                if blend_conf >= 86:
                    model_quality_label = "مرتفع"
                elif blend_conf >= 72:
                    model_quality_label = "متوسط"
                else:
                    model_quality_label = "منخفض"
            
            # âœ… ØªÙˆÙ„ÙŠØ¯ Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø°ÙƒÙŠ
            cached_ai = self.current_data.get('_cached_ai_analysis')
            if isinstance(cached_ai, dict) and cached_ai.get('cache_key') == ai_cache_key:
                insights = cached_ai.get('insights') or {}
            else:
                ...
                self.current_data['_cached_ai_analysis'] = {
            ...
            if self.ml_trainer:
                try:
                    if self.current_data.get('_ml_training_collected_for_cache_key') != ai_cache_key:
                        ...
                        self.current_data['_ml_training_collected_for_cache_key'] = ai_cache_key
                except Exception:
                    pass
            ...
            risk_level = self._decode_mojibake_text(str(fraud.get('risk_level', '') or '')).strip().lower()
            if risk_level in {'منخفض', 'low'}:
                self.fraud_prob_label.config(fg='#28a745')
            elif risk_level in {'متوسط', 'medium'}:
            ...
            concerns_text = "، ".join(concerns) if concerns else "لا توجد مخاوف كبيرة"
            self.failure_concerns_label.config(text=self._t('ai_main_concerns').replace('--', concerns_text))
            
            # Ù„ÙˆÙ† Ø­Ø³Ø¨ Ø§Ù„Ù…Ø®Ø§Ø·Ø±
            f_risk = self._decode_mojibake_text(str(failure.get('risk_level', '') or '')).strip().lower()
            if f_risk in {'منخفض', 'low'}:
                self.failure_3y_label.config(fg='#28a745')
            elif f_risk in {'متوسط', 'medium'}:
            ...
            if model_quality_label != "—":
                g_assessment = f"{g_assessment} | جودة نموذج التوقع: {model_quality_label}"
            ...
            if g_warning:
                self.growth_warning_label.config(text=self._translate_ui_text("تحذير: هناك تذبذب أو ضعف واضح في مسار النمو!"))
            else:
                self.growth_warning_label.config(text=self._translate_ui_text("لا توجد ملاحظات تحذيرية جوهرية حالياً"))
            ...
            # FinancialAnalysisSystem output (if available in current payload)
            fas = (self.current_data.get('financial_analysis_system', {}) or {})
            fas_model = ((fas.get('model') or {}).get('model') if isinstance(fas.get('model'), dict) else None) or '--'
            fas_model_conf = ((fas.get('model') or {}).get('confidence') if isinstance(fas.get('model'), dict) else None)
            fas_score = ((fas.get('score') or {}).get('score') if isinstance(fas.get('score'), dict) else None)
            fas_score_status = ((fas.get('score') or {}).get('status') if isinstance(fas.get('score'), dict) else None) or '--'
            fas_sig = (fas.get('financial_signature') or {}) if isinstance(fas.get('financial_signature'), dict) else {}
            fas_sig_overall = fas_sig.get('overall_signature_score')
            fas_integrity = (fas.get('data_integrity') or {}) if isinstance(fas.get('data_integrity'), dict) else {}
            fas_integrity_summary = (fas_integrity.get('summary') or {}) if isinstance(fas_integrity.get('summary'), dict) else {}
            coverage = fas_integrity_summary.get('coverage_pct')
            kpis_primary = (((fas.get('kpis') or {}).get('kpis') or {}).get('primary') if isinstance(fas.get('kpis'), dict) else None) or []

            if isinstance(fas_model_conf, (int, float)):
                self.fas_model_label.config(text=f"النموذج: {fas_model} (الثقة={fas_model_conf:.2f})")
            else:
                self.fas_model_label.config(text=f"النموذج: {fas_model}")
            if isinstance(fas_score, (int, float)):
                self.fas_score_label.config(text=f"الدرجة: {fas_score:.2f} [{fas_score_status}]")
            else:
                self.fas_score_label.config(text=f"الدرجة: -- [{fas_score_status}]")
            if isinstance(fas_sig_overall, (int, float)):
                self.fas_signature_label.config(text=f"البصمة: إجمالي={fas_sig_overall:.2f}")
            else:
                self.fas_signature_label.config(text="البصمة: --")
            if isinstance(coverage, (int, float)):
                self.fas_integrity_label.config(text=f"سلامة البيانات: التغطية={coverage:.2f}%")
            else:
                self.fas_integrity_label.config(text="سلامة البيانات: --")
            self.fas_kpis_label.config(text=f"أهم مؤشرات الأداء: {', '.join([str(x) for x in kpis_primary]) if kpis_primary else '--'}")

            if self.current_data.get('_cached_ai_chart_key') != ai_cache_key:
                self._schedule_ai_chart_render(insights, cache_key=ai_cache_key)
            self._reset_ai_scroll()
    ...
    def _reset_ai_scroll(self):
        """Ensure AI tab scroll position resets after updates."""
    ...
    def _schedule_ai_chart_render(self, insights, cache_key=None):
        self._pending_ai_chart_payload = {
            'insights': insights,
            'cache_key': cache_key,
        }

        def _runner():
            payload = getattr(self, '_pending_ai_chart_payload', None) or {}
            if not payload:
                return
            try:
                self._render_ai_dashboard_charts(payload.get('insights') or {})
                if self.current_data is not None:
                    self.current_data['_cached_ai_chart_key'] = payload.get('cache_key')
            except Exception:
                self._clear_embedded_chart('_ai_chart_canvas')

        try:
            self.root.after_idle(_runner)
        except Exception:
            _runner()
    ...
    def _manual_train_models(self):
        ...
        try:
            ...
            if results:
                ...
                # Refresh AI lazily so training does not force a heavy rerender.
                try:
                    self._lazy_tab_dirty['ai'] = True
                    self.current_data.pop('_cached_ai_analysis', None)
                    self.current_data.pop('_cached_ai_analysis_key', None)
                    if self._current_notebook_tab_key() == 'ai':
                        self._render_selected_lazy_tab_if_needed()
                except Exception:
                    pass
            else:
    ...
    def _fill_missing_market_ratios(self, market_data):
        ...
        Silent market backfill is intentionally disabled.
        """
        return

    # ---------- Display helpers ----------
    def _mark_lazy_tabs_dirty(self):
        self._lazy_tab_dirty = {
            'comparison': True,
            'forecast': True,
            'ai': True,
    ...
    def _current_notebook_tab_key(self):
        try:
            current_tab = self.notebook.select()
        except Exception:
            return None
        mapping = {
            str(self.raw_tab): 'raw',
            str(self.ratios_tab): 'ratios',
            str(self.strategic_tab): 'strategic',
            str(self.comparison_tab): 'comparison',
            str(self.forecast_tab): 'forecast',
            str(self.ai_analysis_tab): 'ai',
            str(self.chat_tab): 'chat',
            str(self.learning_tab): 'learning',
        }
        return mapping.get(str(current_tab))

    def _render_selected_lazy_tab_if_needed(self):
        tab_key = self._current_notebook_tab_key()
        if not tab_key:
            return
        if not self._lazy_tab_dirty.get(tab_key, False):
            return
        if tab_key == 'comparison':
            self.display_comparison()
        elif tab_key == 'forecast':
            self.display_forecasts()
        elif tab_key == 'ai':
            self.display_ai_analysis()
        self._lazy_tab_dirty[tab_key] = False
    ...
    def display_all(self):
        ...
        # Heavy analytical tabs are rendered lazily when selected.
        self._mark_lazy_tabs_dirty()
        self._render_selected_lazy_tab_if_needed()
        ...
        try:
            ...
            try:
                ...
                if has_price and not market_div_positive:
                    for yy in years:
                        rr = ratios_by_year.setdefault(yy, {})
            ...
            # Apply UI proxy fill to avoid visible N/A and keep Excel aligned with UI.
    ...
    def _get_display_years_range(self, max_years: int = 0):
        """
        Return the user-selected year range exactly as requested.
        (No silent trimming; older years show as empty placeholders if missing.)
        """
        years = self._get_selected_years_range() or self._get_analysis_years_range() or []
    ...
    def _apply_ui_proxy_fill(self, years, ratios_by_year, strategic_by_year=None, sector_profile: str = 'unknown'):
        ...
        carry, or default-fill missing values.
        """
        return
    ...
    def _sanitize_localized_text(self, value, fallback_en=''):
        txt = self._decode_mojibake_text(str(value or '')).strip()
        if not txt:
            if self.current_lang == 'ar' and fallback_en:
                fb = self._translate_financial_item(fallback_en)
                if fb and not self._sanitize_localized_text(fb, '') == '':
                    return fb
            return fallback_en or ('غير متاح' if self.current_lang == 'ar' else 'N/A')
        broken = (
            ('???' in txt)
            or (txt.count('?') >= 3)
            or bool(re.search(r'[ØÙÐÃâðï]{2,}', txt))
        )
        if broken and self.current_lang == 'ar':
            candidates = [
                self._translate_financial_item(fallback_en),
                self._translate_ui_text(fallback_en),
                fallback_en,
            ]
            for cand in candidates:
                cand_txt = self._decode_mojibake_text(str(cand or '')).strip()
                if cand_txt and not (
                    ('???' in cand_txt)
                    or (cand_txt.count('?') >= 3)
                    or bool(re.search(r'[ØÙÐÃâðï]{2,}', cand_txt))
                ):
                    return cand_txt
        return txt
    ...
    def _comparison_metric_catalog(self):
        return [
            {'metric_key': 'revenue', 'label': self._translate_financial_item('Revenue'), 'category': 'Growth', 'source': 'raw', 'keys': ['Revenues', 'RevenueFromContractWithCustomerExcludingAssessedTax', 'Revenue', 'SalesRevenueNet', 'TotalRevenue'], 'higher_better': True, 'aggregation': 'max_positive'},
            {'metric_key': 'net_income', 'label': self._translate_financial_item('Net Income'), 'category': 'Growth', 'source': 'raw', 'keys': ['NetIncomeLoss', 'NetIncome', 'ProfitLoss'], 'higher_better': True},
            {'metric_key': 'operating_cash_flow', 'label': self._translate_financial_item('Operating Cash Flow'), 'category': 'Cash Flow', 'source': 'raw', 'keys': ['NetCashProvidedByUsedInOperatingActivities', 'NetCashProvidedByOperatingActivities'], 'higher_better': True},
            {'metric_key': 'free_cash_flow', 'label': self._translate_financial_item('Free Cash Flow'), 'category': 'Cash Flow', 'source': 'ratio', 'keys': ['free_cash_flow'], 'higher_better': True},
            {'metric_key': 'gross_margin', 'label': self._translate_financial_item('Gross Profit Margin'), 'category': 'Profitability', 'source': 'ratio', 'keys': ['gross_margin'], 'higher_better': True},
            {'metric_key': 'operating_margin', 'label': self._translate_financial_item('Operating Profit Margin'), 'category': 'Profitability', 'source': 'ratio', 'keys': ['operating_margin'], 'higher_better': True},
            {'metric_key': 'net_margin', 'label': self._translate_financial_item('Net Profit Margin'), 'category': 'Profitability', 'source': 'ratio', 'keys': ['net_margin'], 'higher_better': True},
            {'metric_key': 'roic', 'label': self._translate_financial_item('ROIC (Return on Invested Capital)'), 'category': 'Capital Efficiency', 'source': 'ratio', 'keys': ['roic'], 'higher_better': True},
            {'metric_key': 'roe', 'label': self._translate_financial_item('ROE (Return on Equity)'), 'category': 'Capital Efficiency', 'source': 'ratio', 'keys': ['roe'], 'higher_better': True},
            {'metric_key': 'current_ratio', 'label': self._translate_financial_item('Current Ratio'), 'category': 'Liquidity', 'source': 'ratio', 'keys': ['current_ratio'], 'higher_better': True},
            {'metric_key': 'quick_ratio', 'label': self._translate_financial_item('Quick Ratio'), 'category': 'Liquidity', 'source': 'ratio', 'keys': ['quick_ratio'], 'higher_better': True},
            {'metric_key': 'debt_to_equity', 'label': self._translate_financial_item('Debt-to-Equity'), 'category': 'Leverage', 'source': 'ratio', 'keys': ['debt_to_equity'], 'higher_better': False},
            {'metric_key': 'net_debt_ebitda', 'label': self._translate_financial_item('Net Debt / EBITDA'), 'category': 'Leverage', 'source': 'ratio', 'keys': ['net_debt_ebitda'], 'higher_better': False},
            {'metric_key': 'interest_coverage', 'label': self._translate_financial_item('Interest Coverage Ratio'), 'category': 'Leverage', 'source': 'ratio', 'keys': ['interest_coverage'], 'higher_better': True},
    ...
    def _comparison_expert_meta(self, metric_key):
        meta = {
            'revenue': {
                ...
                'why': {'ar': 'يقيس توسع النشاط وحجم الإيرادات عبر الزمن.', 'en': 'Measures expansion of the business and top-line scale.', 'fr': "Mesure l’expansion de l’activité et de la base de revenus."},
            },
            'net_income': {
                'framework': {'ar': 'جودة الأرباح', 'en': 'Earnings Quality', 'fr': 'Qualité des bénéfices'},
                'tool': {'ar': 'نمو صافي الدخل', 'en': 'Net Income Growth', 'fr': 'Croissance du bénéfice net'},
                'why': {'ar': 'يوضح ما إذا كانت الربحية النهائية تتحسن وتتراكم عبر الزمن.', 'en': 'Shows whether bottom-line profitability is compounding over time.', 'fr': 'Montre si la rentabilité nette progresse au fil du temps.'},
            },
            'operating_cash_flow': {
                ...
                'why': {'ar': 'يعتمد الخبراء على التدفق النقدي التشغيلي لاختبار تحول الأرباح إلى سيولة حقيقية عبر الدورة.', 'en': 'Experts rely on operating cash flow to test whether earnings convert into real liquidity through the cycle.', 'fr': "Les experts utilisent le cash-flow opérationnel pour vérifier la conversion des bénéfices en liquidité réelle sur le cycle."},
            },
            'free_cash_flow': {
                'framework': {'ar': 'التدفقات النقدية', 'en': 'Cash Flow', 'fr': 'Flux de trésorerie'},
                'tool': {'ar': 'نمو التدفق النقدي الحر', 'en': 'FCF Growth', 'fr': 'Croissance du FCF'},
                'why': {'ar': 'يركز على النقد المتبقي بعد إعادة الاستثمار، وهو مقياس مؤسسي رئيسي.', 'en': 'Focuses on surplus cash after reinvestment, a key institutional metric.', 'fr': "Mesure le cash disponible après investissement, métrique centrale pour les investisseurs institutionnels."},
            },
            'gross_margin': {
                'framework': {'ar': 'الربحية', 'en': 'Profitability', 'fr': 'Rentabilité'},
                'tool': {'ar': 'قوة التسعير', 'en': 'Pricing Power', 'fr': 'Pouvoir de fixation des prix'},
                'why': {'ar': 'غالبًا ما يشير الهامش الإجمالي المرتفع إلى قوة التسعير أو الكفاءة الهيكلية.', 'en': 'A high gross margin often indicates pricing power or structural efficiency.', 'fr': 'Une marge brute élevée suggère souvent un pouvoir de fixation des prix ou une efficacité structurelle.'},
            },
            'operating_margin': {
                'framework': {'ar': 'الربحية', 'en': 'Profitability', 'fr': 'Rentabilité'},
                'tool': {'ar': 'الكفاءة التشغيلية', 'en': 'Operating Efficiency', 'fr': 'Efficacité opérationnelle'},
                'why': {'ar': 'يستخدم المحللون هامش التشغيل لتقييم الانضباط الإداري وضبط التكاليف.', 'en': 'Analysts use operating margin to judge managerial and cost discipline.', 'fr': "Les analystes utilisent la marge opérationnelle pour juger la discipline des coûts et de la gestion."},
            },
            'net_margin': {
                'framework': {'ar': 'الربحية', 'en': 'Profitability', 'fr': 'Rentabilité'},
                'tool': {'ar': 'تحويل الربح النهائي', 'en': 'Bottom-line Conversion', 'fr': 'Conversion en bénéfice net'},
                'why': {'ar': 'يوضح مقدار الربح النهائي المتبقي من كل وحدة إيراد.', 'en': 'Shows how much final profit remains from each unit of revenue.', 'fr': 'Montre la part du profit final conservée sur chaque unité de revenus.'},
            },
            'roic': {
                'framework': {'ar': 'خلق القيمة', 'en': 'Value Creation', 'fr': 'Création de valeur'},
                'tool': {'ar': 'العائد على رأس المال المستثمر', 'en': 'ROIC', 'fr': 'ROIC'},
                'why': {'ar': 'مقياس خبير أساسي يميز النمو المولّد للقيمة عن النمو المستهلك لرأس المال.', 'en': 'A core expert metric that separates value-creating growth from capital-consuming growth.', 'fr': "Métrique clé pour distinguer la croissance créatrice de valeur de celle qui consomme du capital."},
            },
            'roe': {
                'framework': {'ar': 'العوائد', 'en': 'Returns', 'fr': 'Rendement'},
                'tool': {'ar': 'العائد على حقوق الملكية', 'en': 'ROE', 'fr': 'ROE'},
                'why': {'ar': 'مفيد، لكنه يحتاج تفسيرًا دقيقًا عندما تكون حقوق الملكية مضغوطة.', 'en': 'Useful, but must be interpreted carefully when equity is compressed.', 'fr': "Utile, mais doit être interprété avec prudence lorsque les capitaux propres sont comprimés."},
            },
            'current_ratio': {
                'framework': {'ar': 'السيولة', 'en': 'Liquidity', 'fr': 'Liquidité'},
                'tool': {'ar': 'النسبة المتداولة', 'en': 'Current Ratio', 'fr': 'Ratio courant'},
                'why': {'ar': 'يقيس هامش السيولة القصيرة الأجل في الميزانية.', 'en': 'Measures short-term balance-sheet liquidity buffer.', 'fr': "Mesure le coussin de liquidité à court terme."},
            },
            'quick_ratio': {
                'framework': {'ar': 'السيولة', 'en': 'Liquidity', 'fr': 'Liquidité'},
                'tool': {'ar': 'النسبة السريعة', 'en': 'Quick Ratio', 'fr': 'Ratio de liquidité immédiate'},
                'why': {'ar': 'يستبعد المخزون ويعطي قراءة أكثر صرامة للسيولة.', 'en': 'Excludes inventory and provides a stricter liquidity read.', 'fr': "Exclut le stock et donne une lecture plus stricte de la liquidité."},
            },
            'debt_to_equity': {
                'framework': {'ar': 'الرفع المالي', 'en': 'Leverage', 'fr': 'Levier'},
                'tool': {'ar': 'الديون إلى حقوق الملكية', 'en': 'Debt to Equity', 'fr': 'Dette / Capitaux propres'},
                'why': {'ar': 'مقياس كلاسيكي يقارن تمويل الديون برأس المال المملوك.', 'en': 'Classic leverage lens comparing debt funding to equity capital.', 'fr': "Mesure classique du recours à la dette par rapport aux capitaux propres."},
            },
            'net_debt_ebitda': {
                'framework': {'ar': 'الرفع المالي', 'en': 'Leverage', 'fr': 'Levier'},
                'tool': {'ar': 'صافي الدين إلى EBITDA', 'en': 'Net Debt / EBITDA', 'fr': 'Dette nette / EBITDA'},
                'why': {'ar': 'مفضل لدى المقرضين والمحللين لأنه يربط عبء الدين بالأرباح التشغيلية النقدية.', 'en': 'Favored by lenders and analysts as it links debt load to operating cash earnings.', 'fr': "Préféré par les prêteurs car il relie l’endettement à la capacité opérationnelle de remboursement."},
            },
            'interest_coverage': {
                'framework': {'ar': 'الرفع المالي', 'en': 'Leverage', 'fr': 'Levier'},
                'tool': {'ar': 'تغطية الفوائد', 'en': 'Interest Coverage', 'fr': 'Couverture des intérêts'},
                'why': {'ar': 'يقيس قدرة الأرباح التشغيلية على تغطية عبء الفوائد.', 'en': 'Measures the ability of operating earnings to cover interest burden.', 'fr': "Mesure la capacité des bénéfices opérationnels à couvrir les intérêts."},
            },
            'pe_ratio': {
                'framework': {'ar': 'التقييم', 'en': 'Valuation', 'fr': 'Valorisation'},
                'tool': {'ar': 'مكرر الربحية', 'en': 'P/E', 'fr': 'P/E'},
                'why': {'ar': 'يوضح ما يدفعه السوق مقابل كل وحدة من الأرباح الحالية.', 'en': 'Shows how much the market pays for each unit of current earnings.', 'fr': "Indique ce que le marché paie pour chaque unité de bénéfice actuel."},
            },
            'pb_ratio': {
                'framework': {'ar': 'التقييم', 'en': 'Valuation', 'fr': 'Valorisation'},
                'tool': {'ar': 'مكرر القيمة الدفترية', 'en': 'P/B', 'fr': 'P/B'},
                'why': {'ar': 'يفيد في قياس علاوة السوق فوق القيمة الدفترية المحاسبية.', 'en': 'Useful to judge market premium over accounting book value.', 'fr': "Utile pour juger la prime de marché sur la valeur comptable."},
            },
            'fcf_yield': {
                'framework': {'ar': 'التقييم', 'en': 'Valuation', 'fr': 'Valorisation'},
                'tool': {'ar': 'عائد التدفق النقدي الحر', 'en': 'FCF Yield', 'fr': 'Rendement du FCF'},
                'why': {'ar': 'أداة خبير مباشرة تربط التقييم بتوليد النقد.', 'en': 'A direct expert tool linking valuation to cash generation.', 'fr': "Outil direct reliant la valorisation à la génération de cash."},
            },
        }
        default = {
            'framework': {'ar': 'تحليل عام', 'en': 'General Analysis', 'fr': 'Analyse générale'},
            'tool': {'ar': 'مقياس مقارنة', 'en': 'Comparison Metric', 'fr': 'Indicateur comparatif'},
            'why': {'ar': 'مقياس مساعد للمقارنة والحكم النسبي.', 'en': 'Supporting comparison metric for relative judgment.', 'fr': "Indicateur d’appui pour le jugement comparatif."},
        }
        bundle = meta.get(str(metric_key or '').strip().lower(), default)
        fw_en = bundle['framework'].get('en')
        tool_en = bundle['tool'].get('en')
        why_en = bundle['why'].get('en')
        return {
            'framework': self._sanitize_localized_text(bundle['framework'].get(self.current_lang, fw_en), fallback_en=fw_en),
            'tool': self._sanitize_localized_text(bundle['tool'].get(self.current_lang, tool_en), fallback_en=tool_en),
            'why': self._sanitize_localized_text(bundle['why'].get(self.current_lang, why_en), fallback_en=why_en),
        }
    ...
    def _format_comparison_metric_value(self, metric_key, value):
        ...
        if isinstance(text, str) and text not in {'N/A', '—'}:
            return text
    ...
    def _summarize_comparison_series(self, series, higher_better=True):
        ...
        if directional_change is None:
            ...
            assessment = 'لا يوجد سياق زمني كافٍ لإصدار حكم موثوق.'
        elif directional_change >= 0.10:
            trend_signal = 'IMPROVING'
            assessment = 'الاتجاه يتحسن بشكل واضح مقارنة ببداية الفترة.'
        elif directional_change <= -0.10:
            trend_signal = 'DETERIORATING'
            assessment = 'الاتجاه يتدهور بشكل ملحوظ عبر الفترة.'
        else:
            trend_signal = 'STABLE'
            assessment = 'الاتجاه مستقر نسبيًا عبر الفترة.'
        if isinstance(volatility, (int, float)) and volatility > 0.35:
            assessment += ' لكن التقلبات المرتفعة تستدعي الحذر.'
    ...
    def _build_peer_benchmark_df(self, current_ticker=None, source_snapshot=None):
        try:
            ...
            for key, data in (source or {}).items():
                t = (data.get('company_info', {}) or {}).get('ticker') or key
                if t:
                    available_tickers.append(str(t).upper())
            current_payload = None
            current_sub_sector = None
            for key, data in (source or {}).items():
                t = (data.get('company_info', {}) or {}).get('ticker') or key
                if str(t or '').upper() == str(current_ticker or '').upper():
            ...
            for metric_def in metric_defs:
                ...
                peer_total_count = max(peer_count, int(peers_meta.get('total_count') or 0))
                ...
                expert_meta = self._comparison_expert_meta(metric_def.get('metric_key'))
                if peer_count <= 1:
                    assessment = 'لا توجد أقران كافية لإصدار حكم نسبي قوي.'
                    relative_band = 'INSUFFICIENT_PEERS'
                elif rank == 1:
                    assessment = 'الشركة رائدة ضمن المجموعة المقارنة.'
                    relative_band = 'LEADER'
                elif rank <= max(1, math.ceil(peer_count / 3)):
                    assessment = 'الشركة ضمن الربع الأعلى من المجموعة المقارنة.'
                    relative_band = 'UPPER_TIER'
                elif rank >= max(1, peer_count - math.floor(peer_count / 3)):
                    assessment = 'الشركة ضمن الربع الأدنى نسبيًا مقارنة بالأقران.'
                    relative_band = 'LOWER_TIER'
                else:
                    assessment = 'الشركة في المستوى المتوسط مقارنة بالأقران.'
                ...
                rows.append({
                    ...
                    'Local_Peer_Count': max(0, len(peers_meta.get('local', []))),
                    'Reference_Peer_Count': max(0, len(peers_meta.get('reference', []))),
                    'Reference_Peers': ', '.join(peers_meta.get('reference', [])) if peers_meta.get('reference') else '',
                    'Current_Rank': rank,
    ...
    def display_comparison(self):
        ...
        if not rows:
            empty_text = {
                'ar': 'لا توجد نتائج مقارنة بعد. قم بتحميل أو جلب شركة واحدة على الأقل.',
        ...
        columns = [
            ...
            ('Filing Grade', {'ar': 'جودة التقرير', 'en': 'Filing Grade', 'fr': 'Qualité du dépôt'}),
            ('OutOfRange', {'ar': 'خارج النطاق', 'en': 'Out of Range', 'fr': 'Hors plage'}),
            ('HIGH', {'ar': 'عالٍ', 'en': 'HIGH', 'fr': 'ÉLEVÉ'}),
            ('MEDIUM', {'ar': 'متوسط', 'en': 'MEDIUM', 'fr': 'MOYEN'}),
            ('LOW', {'ar': 'منخفض', 'en': 'LOW', 'fr': 'FAIBLE'}),
            ('REJECTED', {'ar': 'مرفوض', 'en': 'REJECTED', 'fr': 'REJETÉ'}),
            ('Top Rejection Reasons', {'ar': 'أبرز أسباب الرفض', 'en': 'Top Rejection Reasons', 'fr': 'Principales raisons de rejet'}),
            ('Top Validator Flags', {'ar': 'أبرز إشارات التحقق', 'en': 'Top Validator Flags', 'fr': 'Principaux drapeaux de validation'}),
        ]
        ...
        summary_text = {
            'ar': f"نتائج المقارنة المؤسسية: {len(rows)} شركة | خارج النطاق: {out_of_range_count} | اختر شركة لعرض التفاصيل.",
    ...
    def _on_comparison_select(self, _event=None):
        ...
        if not sel:
            return
        ticker = str(sel[0] or '').strip()
        if ticker:
            key = self._resolve_company_key(ticker)
            if key in (self.multi_company_data or {}):
                self.current_data = self.multi_company_data[key]
                try:
                    if hasattr(self, 'company_selector_var'):
                        self.company_selector_var.set(str((self.current_data.get('company_info', {}) or {}).get('ticker') or ticker))
                    names = list(self.companies_listbox.get(0, tk.END))
                    upper_map = {str(n).upper(): i for i, n in enumerate(names)}
                    idx = upper_map.get(str(ticker).upper())
                    if idx is not None:
                        self.companies_listbox.selection_clear(0, tk.END)
                        self.companies_listbox.selection_set(idx)
                        self.companies_listbox.activate(idx)
                ...
                try:
                    ...
                    self._update_workspace_context()
        self._render_comparison_details(ticker or sel[0])
    ...
    def _render_peer_expert_comparison_details(self, ticker, details):
        ...
        columns = [
            ...
            ('Metric_Label', {'ar': 'المقياس', 'en': 'Metric', 'fr': 'Indicateur'}),
            ('Current_Value', {'ar': 'قيمة الشركة', 'en': 'Company Value', 'fr': 'Valeur société'}),
            ('Peer_Median', {'ar': 'وسيط الأقران', 'en': 'Peer Median', 'fr': 'Médiane pairs'}),
            ('Current_Rank', {'ar': 'الترتيب', 'en': 'Rank', 'fr': 'Rang'}),
            ('Best_Ticker', {'ar': 'الأفضل', 'en': 'Best', 'fr': 'Meilleur'}),
            ('Relative_Assessment', {'ar': 'التقييم النسبي', 'en': 'Relative Assessment', 'fr': 'Jugement relatif'}),
        ]
        col_ids = [c[0] for c in columns]
        ...
        if df is not None and not df.empty:
            for _, row in df.iterrows():
                ...
                framework_txt = self._sanitize_localized_text(row.get('Framework'), fallback_en='General')
                tool_txt = self._sanitize_localized_text(row.get('Expert_Tool'), fallback_en=str(metric_key or 'Metric'))
                metric_label_fallback = self._translate_financial_item(str(metric_key or 'Metric')) if self.current_lang == 'ar' else str(metric_key or 'Metric')
                metric_label_txt = self._sanitize_localized_text(row.get('Metric_Label'), fallback_en=metric_label_fallback)
                rel_assess_txt = self._comparison_status_text(row.get('Relative_Assessment'), kind='relative')
                ...
                if band in {'LEADER', 'UPPER_TIER'}:
                    leaders.append(metric_label_txt)
        ...
        if self.current_lang == 'ar':
            ...
            if leaders:
                summary_lines.append("أقوى المقاييس: " + '، '.join(leaders[:4]))
            if laggards:
                summary_lines.append("مقاييس تحتاج تحسينًا: " + '، '.join(laggards[:4]))
            summary_lines.append(
                f"أعلام التحقق: {self._comparison_status_text(details.get('top_flags') or 'NONE', kind='relative')} | "
                f"أسباب الرفض: {self._comparison_status_text(details.get('top_reasons') or 'NONE', kind='relative')}"
            )
        else:
            ...
            summary_lines.append(
                f"Validator flags: {self._comparison_status_text(details.get('top_flags') or 'NONE', kind='relative')} | "
                f"Rejection reasons: {self._comparison_status_text(details.get('top_reasons') or 'NONE', kind='relative')}"
            )
        self.comparison_diag_text.delete('1.0', 'end')
    ...
    def _render_time_series_expert_comparison_details(self, ticker, details):
        ...
        columns = [
            ...
            ('CAGR_%', {'ar': 'النمو السنوي', 'en': 'CAGR', 'fr': 'TCAC'}),
            ('Trend_Signal', {'ar': 'الاتجاه', 'en': 'Trend', 'fr': 'Tendance'}),
            ('Assessment', {'ar': 'التقييم', 'en': 'Assessment', 'fr': 'Jugement'}),
        ]
        col_ids = [c[0] for c in columns]
        self.comparison_detail_tree.config(columns=col_ids)
        for c, label_map in columns:
        ...
        if df is not None and not df.empty:
            for _, row in df.iterrows():
                ...
                metric_label_fallback = self._translate_financial_item(str(metric_key or 'Metric')) if self.current_lang == 'ar' else str(metric_key or 'Metric')
                metric_label_txt = self._sanitize_localized_text(row.get('Metric_Label'), fallback_en=metric_label_fallback)
                trend_txt = self._comparison_status_text(row.get('Trend_Signal'), kind='trend')
                assessment_txt = self._comparison_status_text(row.get('Assessment'), kind='trend')
                self.comparison_detail_tree.insert(
                    '',
                    'end',
                    values=(
                ...
                if signal == 'IMPROVING':
                    improving.append(metric_label_txt)
                elif signal in {'DETERIORATING', 'VOLATILE'}:
                    deteriorating.append(metric_label_txt)

        summary_lines = []
        if self.current_lang == 'ar':
            summary_lines.append(f"مقارنة اتجاهات زمنية احترافية للشركة {ticker}.")
            if improving:
                summary_lines.append("المقاييس المتحسنة: " + '، '.join(improving[:4]))
            if deteriorating:
                summary_lines.append("المقاييس التي تحتاج متابعة: " + '، '.join(deteriorating[:4]))
            summary_lines.append(
                f"أعلام التحقق: {self._comparison_status_text(details.get('top_flags') or 'NONE', kind='trend')} | "
                f"أسباب الرفض: {self._comparison_status_text(details.get('top_reasons') or 'NONE', kind='trend')}"
            )
        else:
            ...
            summary_lines.append(
                f"Validator flags: {self._comparison_status_text(details.get('top_flags') or 'NONE', kind='trend')} | "
                f"Rejection reasons: {self._comparison_status_text(details.get('top_reasons') or 'NONE', kind='trend')}"
            )
        self.comparison_diag_text.delete('1.0', 'end')
        self.comparison_diag_text.insert('1.0', '\n'.join(summary_lines))
    ...
    def display_raw_data(self):
        if not self.current_data:
            ...
            return
        # Enforce pure table mode; never show the implicit Treeview column (#0).
        try:
            self.raw_tree.configure(show='headings')
            self.raw_tree.column('#0', width=0, stretch=False)
            self.raw_tree.heading('#0', text='')
        except Exception:
            pass
        for i in self.raw_tree.get_children():
            self.raw_tree.delete(i)
        ...
        def _estimate_numeric(series_map, year):
            return None, False

        def _reset_raw_scroll():
            try:
                self.raw_tree.update_idletasks()
                self.raw_tree.xview_moveto(0.0)
                self.raw_tree.yview_moveto(0.0)
            except Exception:
                pass
        ...
        for item in catalog:
            if str(item.get('title')) == selected_layer:
                ...
                break
        if not selected_key:
            norm_layer = self._normalize_term_for_lookup(selected_layer)
            if 'layer 2' in norm_layer or 'الطبقة 2' in norm_layer:
                selected_key = 'layer2_by_year'
                selected_source = 'MARKET'
            elif 'layer 3' in norm_layer or 'الطبقة 3' in norm_layer:
                selected_key = 'layer3_by_year'
                selected_source = 'MACRO'
            elif 'layer 4' in norm_layer or 'الطبقة 4' in norm_layer:
        ...
        if sec_view_mode == 'inputs':
            ...
            self.raw_tree.tag_configure('zebra_even', background=PALETTE['surface_soft'], foreground=PALETTE['text'])
            self.raw_tree.tag_configure('zebra_odd', background=PALETTE['surface'], foreground=PALETTE['text'])
            year_col_w = 80 if len(years) > 12 else 120
            for c in cols:
                self.raw_tree.heading(c, text=c)
                if c == self._t('raw_col_category'):
                    self.raw_tree.column(c, width=180, anchor='center', stretch=True, minwidth=140)
                elif c == self._t('raw_col_normalized'):
                    self.raw_tree.column(c, width=420, anchor='w', stretch=True, minwidth=240)
                elif c == self._t('raw_col_used_in'):
                    self.raw_tree.column(c, width=300, anchor='w', stretch=True, minwidth=200)
                elif c == self._t('raw_col_source'):
                    self.raw_tree.column(c, width=170, anchor='center', stretch=False, minwidth=120)
                else:
                    self.raw_tree.column(c, width=year_col_w, anchor='center', stretch=False, minwidth=70)
            ...
            input_spec = [
                ...
                ("مقاييس الميزانية", [
                    ("الأصول", [('SEC', 'Assets'), ('SEC', 'TotalAssets')], "ROA, debt_to_assets, balance_check"),
                    ("الالتزامات", [('SEC', 'Liabilities'), ('SEC', 'TotalLiabilities')], "balance_check"),
                ...
                ("التدفقات النقدية", [
                    ...
                    ("النفقات الرأسمالية", [('SEC', 'PaymentsToAcquirePropertyPlantAndEquipment'), ('RATIO', 'capital_expenditures')], "free_cash_flow"),
                ...
                ("محددات المخاطر", [
                    ("معدل الخلو من المخاطر", [('MACRO', 'macro:risk_free_rate'), ('MACRO', 'risk_free_rate')], "cost_of_equity, WACC"),
                    ("علاوة مخاطر الأسهم", [('MACRO', 'macro:equity_risk_premium'), ('MACRO', 'equity_risk_premium')], "cost_of_equity, WACC"),
            ...
            for parent_label, children in input_spec:
                ...
                for child_label, candidates, used_in in children:
                    ...
                    for y in years:
                        ...
                        if v is not None:
                            ...
                            if isinstance(est, (int, float)):
                                ...
                                used_estimate = used_estimate or est_used
                            else:
                                vals.append('')
                    source_label = ''
                    if source_counts:
                    ...
                    tags = list(_raw_tags('child_row'))
                    if used_estimate:
                        tags.append('estimated_row')
                    self.raw_tree.insert('', 'end', values=child_row, tags=tuple(tags))
        ...
        sec_csv_raw = ((self.current_data.get('institutional_saved_files', {}) or {}).get('sec_official_statement'))
        ticker_now = str((self.current_data.get('company_info', {}) or {}).get('ticker', '')).strip().upper()
        sec_csv = None
        if ticker_now:
            trusted_dir = (Path('exports') / 'institutional').resolve()
            trusted_name = f"{ticker_now}_SEC_Official_Statement.csv"
            trusted_path = trusted_dir / trusted_name
            # Prefer canonical per-ticker institutional export.
            if trusted_path.exists():
                sec_csv = str(trusted_path)
            elif isinstance(sec_csv_raw, str) and sec_csv_raw:
                # Accept saved path only if it is exactly the trusted per-ticker file.
                try:
                    candidate = Path(sec_csv_raw).resolve()
                    if (
                        candidate.exists()
                        and candidate.name.upper() == trusted_name.upper()
                        and trusted_dir in candidate.parents
                    ):
                        sec_csv = str(candidate)
                except Exception:
                    sec_csv = None
        if (
            selected_key == 'layer1_by_year'
            ...
            # Be robust to BOM/encoding variants in the first column name.
            item_col = None
            unit_col_in = None
            def _norm_col_name(col_name: str) -> str:
                return re.sub(r'[^a-z0-9]', '', str(col_name or '').strip().lower().lstrip('\ufeff'))

            for f in fields:
                norm_f = _norm_col_name(f)
                if item_col is None and norm_f in {
                    'lineitem', 'item', 'label', 'rawlabel', 'normalizedlabel',
                    'normalizeditem', 'concept', 'description', 'account'
                }:
                    item_col = f
                if unit_col_in is None and norm_f in {'unit', 'uom'}:
                    unit_col_in = f
            if item_col is None and fields:
                # Fallback: pick the most text-like non-year column.
                non_year_cols = [f for f in fields if not re.search(r'20\d{2}', str(f or ''))]
                best_col = None
                best_score = -1.0
                sample_rows = rows[: min(60, len(rows))]
                for c in non_year_cols:
                    norm_c = _norm_col_name(c)
                    if norm_c in {'unit', 'uom', 'source'}:
                        continue
                    vals = [str((r or {}).get(c, '') or '').strip() for r in sample_rows]
                    non_empty = [v for v in vals if v]
                    if not non_empty:
                        continue
                    text_like = 0
                    for v in non_empty:
                        v_compact = v.replace(',', '').replace('.', '').replace('-', '')
                        if not v_compact.isdigit():
                            text_like += 1
                    score = (text_like / max(1, len(non_empty)))
                    if score > best_score:
                        best_score = score
                        best_col = c
                item_col = best_col or fields[0]

            # Hard file-quality guardrails before rendering in official mode.
            date_cols_in_header = [c for c in fields if re.fullmatch(r"\s*20\d{2}\s*", str(c or ''))]
            if len(date_cols_in_header) < 2:
                sec_csv = None

            # If the resolved item column still looks malformed (e.g. "." with "Ac/Tc"),
            # fallback to canonical Layer1 display for correctness.
            if item_col is not None:
                sample_labels = [
                    str((r or {}).get(item_col, '') or '').strip()
                    for r in rows[: min(40, len(rows))]
                ]
                weak = 0
                for lbl in sample_labels:
                    if self._label_looks_truncated_or_weak(lbl):
                        weak += 1
                if sample_labels and (weak / max(1, len(sample_labels))) >= 0.3:
                    sec_csv = None
        ...
        if (
            ...
            # Fallback to canonical rendering when trusted SEC statement file is unavailable/invalid.
            sec_view_mode = 'canonical'
        ...
        if (
            ...
            # Note: item_col can legally be an empty-string header ("") in some SEC CSV exports.
            # Treat that as a valid column key.
            if item_col is not None:
                date_cols = [c for c in fields if c != item_col and re.search(r'20\d{2}', str(c))]
                ...
                order = []
                for r in rows:
                    raw_item = self._decode_mojibake_text(str(r.get(item_col) or '').strip())
                    raw_item = re.sub(r"\s+", " ", raw_item).strip()
                    if not raw_item:
                        continue
                    # Run expensive rescue only when label looks clearly broken.
                    if self._label_looks_truncated_or_weak(raw_item):
                        raw_item = self._rescue_official_sec_label(
                            raw_item,
                            r,
                            date_cols,
                            layer1_active,
                    ...
                    if item_key not in merged:
                        merged[item_key] = {item_col: raw_item, '_label_source': 'csv', '_unit': ''}
                        for d in date_cols:
                            merged[item_key][d] = ''
                        ...
                        if merged[item_key].get('_label_source') != 'csv':
                            merged[item_key][item_col] = self._prefer_display_label(
                                merged[item_key].get(item_col),
                                raw_item,
                            )
                    unit_v = str(r.get(unit_col_in, '') or '').strip() if unit_col_in else ''
                    if not merged[item_key].get('_unit') and unit_v:
                        merged[item_key]['_unit'] = unit_v
                    for d in date_cols:
                ...
                if sec_view_mode == 'canonical' and isinstance(layer1_active, dict) and date_cols:
                    ...
                    for concept in sorted(concept_pool):
                        ...
                        if c_key not in merged:
                            merged[c_key] = {item_col: c_label, '_label_source': 'layer1', '_unit': 'USD (millions)'}
                            for d in date_cols:
                                merged[c_key][d] = ''
                            order.append(c_key)
                        else:
                            ...
                            if merged[c_key].get('_label_source') != 'csv':
                                merged[c_key][item_col] = self._prefer_display_label(
                                    merged[c_key].get(item_col),
                                    c_label,
                                )
                        for d in date_cols:
                            try:
                ...
                # If official labels are mostly truncated, switch to canonical view immediately.
                try:
                    labels_now = [str((rr or {}).get(item_col, '') or '').strip() for rr in rows]
                    labels_now = [x for x in labels_now if x]
                    weak_now = sum(1 for x in labels_now if self._label_looks_truncated_or_weak(x))
                    weak_ratio_now = (weak_now / max(1, len(labels_now))) if labels_now else 1.0
                except Exception:
                    weak_ratio_now = 1.0
                if weak_ratio_now >= 0.20:
                    try:
                        sec_view_mode = 'canonical'
                        if hasattr(self, '_sec_view_mode_var'):
                            self._sec_view_mode_var.set('canonical')
                        if hasattr(self, 'sec_view_mode_display_var'):
                            self.sec_view_mode_display_var.set(self._t('sec_view_mode_canonical'))
                    except Exception:
                        pass
                    return self.display_raw_data()
                cols = [self._t('raw_col_item')] + date_cols + [self._t('raw_col_unit')]
                self.raw_tree.config(columns=cols)
                self.raw_tree.tag_configure('parent_row', font=FONTS['label'])
                self.raw_tree.tag_configure('child_row', font=FONTS['tree'])
                self.raw_tree.tag_configure('zebra_even', background=PALETTE['surface_soft'], foreground=PALETTE['text'])
                self.raw_tree.tag_configure('zebra_odd', background=PALETTE['surface'], foreground=PALETTE['text'])
                year_col_w = 80 if len(date_cols) > 12 else 120
                for c in cols:
                    self.raw_tree.heading(c, text=c if c in (self._t('raw_col_item'), self._t('raw_col_unit')) else self._translate_financial_item(c))
                    if c == self._t('raw_col_item'):
                        self.raw_tree.column(c, width=620, anchor='w', stretch=True, minwidth=320)
                    elif c == self._t('raw_col_unit'):
                        self.raw_tree.column(c, width=170, anchor='center', stretch=False, minwidth=130)
                        ...
                        self.raw_tree.column(c, width=year_col_w, anchor='center', stretch=False, minwidth=70)
                def _infer_unit_from_label(label_text: str) -> str:
                    t = str(label_text or '').lower()
                    if self.current_lang == 'ar':
                        if ('per share' in t) or ('eps' in t) or ('سهم' in t):
                            return 'دولار/سهم'
                ...
                for r in rows:
                    ...
                    for idx, c in enumerate(date_cols, start=1):
                        if str(c).isdigit() and (vals[idx] is None or str(vals[idx]).strip() == ''):
                            est, est_used = _estimate_numeric(series_map, int(c))
                            if isinstance(est, (int, float)):
                                vals[idx] = "˜" + fmt_value(est)
                                used_estimate = used_estimate or est_used
                    tag = 'parent_row' if self._is_parent_line_item(raw_label, vals[1:1+len(date_cols)]) else 'child_row'
                    tags = list(_raw_tags(tag))
                    if used_estimate:
                        tags.append('estimated_row')
                    self.raw_tree.insert('', 'end', values=tuple(vals), tags=tuple(tags))
            else:
                cols = fields
                self.raw_tree.config(columns=cols)
                self.raw_tree.tag_configure('parent_row', font=FONTS['label'])
                ...
                for r in rows:
                    ...
                    if values:
                        values[0] = self._sanitize_localized_text(
                            self._translate_financial_item(raw_label),
                            raw_label,
                        )
        ...
        if selected_source in ('MARKET', 'MACRO', 'YAHOO'):
            ...
            for c in cols:
                self.raw_tree.heading(c, text=c)
                if c == self._t('raw_col_item'):
                    self.raw_tree.column(c, width=380, anchor='w', stretch=True)
                elif c in (self._t('raw_col_unit'), self._t('raw_col_source')):
                    self.raw_tree.column(c, width=180, anchor='center', stretch=False)
                else:
                    self.raw_tree.column(c, width=year_col_w, anchor='center', stretch=False)
            ...
            for field_name in sorted(fields_set):
                ...
                for y in years:
                    ...
                    v = normalized_v if normalized_v is not None else payload_v
                    series_map[y] = v
                    if v is None or (isinstance(v, str) and v.strip().lower() in {'nan', 'none', 'null'}):
                        est, est_used = _estimate_numeric(series_map, y)
                        if isinstance(est, (int, float)):
                            row.append("˜" + fmt_value(est))
                            used_estimate = used_estimate or est_used
                        else:
                            row.append('')
                    else:
                ...
                if used_estimate:
                    tags.append('estimated_row')
                self.raw_tree.insert('', 'end', values=row, tags=tuple(tags))
        ...
        if all_available_years:
            if self.current_lang == 'ar':
                info_msg += f"\n?? ??????? ??????? ???????: {min(all_available_years)} - {max(all_available_years)}"
            elif self.current_lang == 'fr':
            ...
            if requested_end > max(all_available_years):
                if self.current_lang == 'ar':
                    ...
                    info_msg += f"\n?? Certaines années sont hors du jeu de données source et seront affichées en —."
                else:
                    info_msg += f"\n?? Some years are outside available source data and will be shown as —."
        ...
        if selected_key == 'layer3_by_year':
            ...
            for c in cols:
                self.raw_tree.heading(c, text=c)
                if c == self._t('raw_col_category'):
                    self.raw_tree.column(c, width=180, stretch=True)
                elif c == self._t('raw_col_normalized'):
                    self.raw_tree.column(c, width=320, stretch=True)
                else:
                    self.raw_tree.column(c, width=year_col_w, stretch=False)
            ...
            for category, normalized, concept_key in sorted(parsed_items, key=lambda t: (t[0], t[1])):
                ...
                used_estimate = False
                series_map = {y: (active_by_year.get(y, {}) or {}).get(concept_key) for y in years}
                for y in years:
                    v = series_map.get(y)
                    if v is None or (isinstance(v, str) and v.strip().lower() in {'nan', 'none', 'null'}):
                        est, est_used = _estimate_numeric(series_map, y)
            ...
            for c in cols:
                self.raw_tree.heading(c, text=c)
                if c == first_col:
                    self.raw_tree.column(c, width=360, stretch=True)
                elif c == self._t('raw_col_unit'):
                    self.raw_tree.column(c, width=140, stretch=False)
                else:
                    self.raw_tree.column(c, width=year_col_w, stretch=False)
            ...
            if selected_key == 'layer1_by_year':
                ...
                for sem_key in order:
                    ...
                    for y in years:
                        v = meta['vals'].get(y)
                        if v is None or (isinstance(v, str) and v.strip().lower() in {'nan', 'none', 'null'}):
                            est, est_used = _estimate_numeric(meta['vals'], y)
                            if isinstance(est, (int, float)):
                                row.append("˜" + fmt_value(est))
                                used_estimate = used_estimate or est_used
                            else:
                            ...
                            row.append(fmt_value(v))
                    row.append(_infer_unit_from_label(raw_label))
                ...
                for concept in display_concepts:
                    row = [concept]
                    used_estimate = False
                    series_map = {y: (active_by_year.get(y, {}) or {}).get(concept) for y in years}
                    for y in years:
                        v = series_map.get(y)
                        if v is None or (isinstance(v, str) and v.strip().lower() in {'nan', 'none', 'null'}):
                            est, est_used = _estimate_numeric(series_map, y)
                            ...
                            row.append(fmt_value(v))
                    row.append(_infer_unit_from_label(concept))
    ...
    def _sector_profile_to_display(self, sector_profile: str) -> str:
        ...
        mapping = {
            ...
            'software_saas': {'ar': 'برمجيات / سحابي', 'en': 'Software / SaaS', 'fr': 'Logiciels / SaaS'},
            'hardware_platform': {'ar': 'منصات عتاد', 'en': 'Hardware Platform', 'fr': 'Plateforme matérielle'},
            'semiconductor_fabless': {'ar': 'أشباه موصلات بدون مصانع', 'en': 'Fabless Semiconductors', 'fr': 'Semi-conducteurs fabless'},
            'tech': {'ar': 'تقنية', 'en': 'Technology', 'fr': 'Technologie'},
            'industrial': {'ar': 'صناعي', 'en': 'Industrial', 'fr': 'Industrie'},
            'bank': {'ar': 'مصارف', 'en': 'Banking', 'fr': 'Banque'},
            ...
            'insurance_life': {'ar': 'تأمين حياة', 'en': 'Life Insurance', 'fr': 'Assurance vie'},
            'insurance_pc': {'ar': 'تأمين ممتلكات وحوادث', 'en': 'P&C Insurance', 'fr': 'Assurance IARD'},
            'insurance_broker': {'ar': 'وساطة تأمين', 'en': 'Insurance Broker', 'fr': 'Courtage en assurance'},
            'energy': {'ar': 'طاقة', 'en': 'Energy', 'fr': 'Énergie'},
            'integrated_oil': {'ar': 'نفط متكامل', 'en': 'Integrated Oil', 'fr': 'Pétrole intégré'},
            'utilities': {'ar': 'مرافق', 'en': 'Utilities', 'fr': 'Services publics'},
            'healthcare': {'ar': 'رعاية صحية', 'en': 'Healthcare', 'fr': 'Santé'},
            'consumer': {'ar': 'استهلاكي', 'en': 'Consumer', 'fr': 'Consommation'},
            'consumer_staples': {'ar': 'سلع استهلاكية أساسية', 'en': 'Consumer Staples', 'fr': 'Biens de consommation de base'},
            'ev_automaker': {'ar': 'صانع سيارات كهربائية', 'en': 'EV Automaker', 'fr': 'Constructeur VE'},
            'realestate': {'ar': 'عقارات', 'en': 'Real Estate', 'fr': 'Immobilier'},
            'telecom': {'ar': 'اتصالات', 'en': 'Telecom', 'fr': 'Télécom'},
            'materials': {'ar': 'مواد', 'en': 'Materials', 'fr': 'Matériaux'},
            'unknown': {'ar': 'غير معروف', 'en': 'Unknown', 'fr': 'Inconnu'},
            '': {'ar': 'غير معروف', 'en': 'Unknown', 'fr': 'Inconnu'},
        }
    ...
    def _comparison_filing_grade_text(self, filing_grade: str) -> str:
        ...
        mapping = {
            ...
            'N/A': {'ar': 'غير متاح', 'en': 'Unavailable', 'fr': 'Indisponible'},
    ...
    def _comparison_status_text(self, value: str, kind: str = 'relative') -> str:
        txt = self._decode_mojibake_text(str(value or '')).strip().upper()
        if not txt:
            return self._translate_ui_text('غير متاح')
        mapping = {
            'relative': {
                'LEADER': {'ar': 'متصدر', 'en': 'Leader', 'fr': 'Leader'},
                'UPPER_TIER': {'ar': 'فئة عليا', 'en': 'Upper tier', 'fr': 'Haut de gamme'},
                'LOWER_TIER': {'ar': 'فئة دنيا', 'en': 'Lower tier', 'fr': 'Bas de gamme'},
                'NO_PEER_SET_OR_NO_COMPARABLE_VALUES': {'ar': 'لا توجد أقران كافيين', 'en': 'No peers', 'fr': 'Pas de pairs'},
                'NO_EXPERT_COMPARISON_DATA': {'ar': 'لا توجد بيانات مقارنة', 'en': 'No comparison data', 'fr': 'Aucune donnée'},
                'MISSING_LATEST_VALUE': {'ar': 'القيمة الأخيرة مفقودة', 'en': 'Latest value missing', 'fr': 'Dernière valeur manquante'},
                'LATEST_VALUE_GAP': {'ar': 'فجوة في آخر قيمة', 'en': 'Latest value gap', 'fr': 'Écart de dernière valeur'},
                'NONE': {'ar': 'لا يوجد', 'en': 'None', 'fr': 'Aucun'},
                'UNKNOWN': {'ar': 'غير معروف', 'en': 'Unknown', 'fr': 'Inconnu'},
            },
            'trend': {
                'IMPROVING': {'ar': 'يتحسن', 'en': 'Improving', 'fr': 'En amélioration'},
                'STABLE': {'ar': 'مستقر', 'en': 'Stable', 'fr': 'Stable'},
                'DETERIORATING': {'ar': 'يتدهور', 'en': 'Deteriorating', 'fr': 'Se dégrade'},
                'WEAK': {'ar': 'ضعيف', 'en': 'Weak', 'fr': 'Faible'},
                'STRONG': {'ar': 'قوي', 'en': 'Strong', 'fr': 'Fort'},
                'NO_TIME_SERIES_COMPARISON_DATA': {'ar': 'لا توجد بيانات مقارنة زمنية', 'en': 'No time-series comparison data', 'fr': 'Aucune donnée temporelle'},
                'NONE': {'ar': 'لا يوجد', 'en': 'None', 'fr': 'Aucun'},
                'UNKNOWN': {'ar': 'غير معروف', 'en': 'Unknown', 'fr': 'Inconnu'},
            },
        }.get(kind, {})
        bundle = mapping.get(txt)
        if bundle:
            return bundle.get(self.current_lang, bundle.get('en', txt))
        return self._translate_ui_text(value or txt)
    ...
    def _parent_profile_from_sub_sector(sub_sector: str) -> str:
        ...
        if s in {'commercial_bank'}:
            return 'bank'
        if s in {'investment_bank'}:
            return 'investment_bank'
        if s in {'insurance_life', 'insurance_pc', 'insurance_broker'}:
            return 'insurance'
        if s in {
            'software_saas',
            'hardware_platform',
    ...
    def _filter_imported_year_dict_by_sector(self, year_dict, sector_profile: str):
        sector = str(sector_profile or '').strip().lower()
        if not isinstance(year_dict, dict) or not sector:
            return year_dict or {}
        ...
        bank_only_markers = (
            ...
            'ProvisionForCreditLosses', 'AllowanceForCreditLosses',
            'FederalFundsSold', 'NoninterestBearingDeposits',
            'DepositLiabilities', 'InterestIncomeOperating',
            'InterestAndDividendIncomeOperating', 'InterestIncomeNet',
        )
        industrial_only_markers = (
            'Inventory', 'CostOfRevenue', 'GrossProfit',
            'ResearchAndDevelopmentExpense', 'SellingGeneralAndAdministrativeExpense',
        ...
        out = {}
        for y, row in (year_dict or {}).items():
            if not isinstance(row, dict):
                out[y] = row
                continue
            filtered = {}
            for k, v in row.items():
                ks = str(k)
                if sector in {'bank', 'commercial_bank', 'investment_bank'}:
                    if any(marker in ks for marker in industrial_only_markers):
                        continue
                else:
                    if any(marker in ks for marker in bank_only_markers):
                        continue
                filtered[k] = v
            out[y] = filtered
        return out
    ...
    def _extract_excel_import_metadata(self, ticker_guess, comparison_df=None, methodology_df=None):
        ...
        if meta.get('sub_sector_profile'):
            meta['sector_profile'] = self._parent_profile_from_sub_sector(meta.get('sub_sector_profile'))
        return meta
    ...
    @classmethod
    def _extract_revenue_investment_bank(cls, raw_sec: dict):
        for label in cls.INVESTMENT_BANK_REVENUE_LABELS:
            try:
                val = raw_sec.get(label) if isinstance(raw_sec, dict) else None
                if val is not None and not (isinstance(val, float) and (math.isnan(val) or math.isinf(val))):
                    return float(val)
            except Exception:
                continue
        try:
            nii = (raw_sec.get("NetInterestIncome", 0) or 0) if isinstance(raw_sec, dict) else 0
            nir = (raw_sec.get("NonInterestRevenue", 0) or 0) if isinstance(raw_sec, dict) else 0
            if float(nii) + float(nir) > 0:
    ...
    def _extract_revenue_by_sector(self, row: dict, sector_profile: str):
        ...
        for key in preferred:
            rk = lower_map.get(str(key).strip().lower(), key)
            try:
                val = row.get(rk)
    ...
    def _merge_financial_analysis_system_ratios(self, ratios_by_year):
        """
        Overlay trusted ratios produced by FinancialAnalysisSystem onto the local
        ratio map without overwriting existing concrete values.
        """
        merged = {}
        ...
        for year, metric_map in (fas_ratios or {}).items():
            try:
                yy = int(year)
            except Exception:
                continue
            rr = merged.setdefault(yy, {})
            reasons = rr.get('_ratio_reasons')
            if not isinstance(reasons, dict):
                rr['_ratio_reasons'] = {}
                reasons = rr['_ratio_reasons']
            for fas_key, ratio_key in fas_map.items():
                record = (metric_map or {}).get(fas_key)
                if isinstance(record, dict):
                    raw_val = record.get('value')
                else:
                    raw_val = record
                num_val = self._safe_excel_number(raw_val)
                current_val = self._safe_excel_number(rr.get(ratio_key))
                if num_val is None or current_val is not None:
                    continue
                rr[ratio_key] = float(num_val)
                rr[f'{ratio_key}_source'] = 'FAS'
                reasons.pop(ratio_key, None)
        return merged
    ...
    def _get_sector_strategic_export_keys(self, sector_profile: str):
        ...
        metric_packs = {
            ...
            'bank': [
                ...
                'PE_Ratio', 'PB_Ratio', 'Dividend_Yield', 'Beta', 'WACC',
            ],
            'investment_bank': [
                'Capital_Ratio_Proxy', 'Net_Income_to_Assets', 'Equity_Ratio', 'Bank_Efficiency_Ratio',
                'ROA', 'ROE', 'Net_Margin', 'Cost_of_Debt', 'Credit_Rating', 'Credit_Rating_Score', 'Warning_Signal',
        ...
        for ratio_id in ratio_export_keys:
            ...
            for year in years:
                ...
                ratio_audit_rows.append({
                    ...
                    'Display': format_ratio_value(ratio_id, value).get('display_text') if is_computed else f"— ({reason or 'NOT_COMPUTABLE'})",
    ...
    def display_ratios(self):
        ...
        if not years:
            return
        year_col_w = 90 if len(years) > 15 else 130
        ...
        for c in cols:
            ...
            if c == ratio_col:
                self.ratios_tree.column(c, width=280, anchor='w', stretch=True)
            elif c == explanation_col:
                self.ratios_tree.column(c, width=220, stretch=True)
            else:
                self.ratios_tree.column(c, width=year_col_w, stretch=False)
        ...
        def fmt_ratio_contract(m, contract):
            ...
            reason_short = {
                'MISSING_SEC_CONCEPT': 'مفهوم SEC مفقود',
                'MISSING_MARKET_DATA': 'بيانات سوق مفقودة',
                'MISSING_REQUIRED_LAYER': 'طبقة مطلوبة مفقودة',
                'ZERO_DENOMINATOR': 'المقام=0',
                'INSUFFICIENT_HISTORY': 'سجل تاريخي غير كافٍ',
                'DATA_NOT_APPLICABLE': 'غير منطبق',
                'NOT_APPLICABLE_FOR_SECTOR': 'غير منطبق للقطاع',
                'NOT_COMPUTABLE': 'غير قابل للحساب',
                'MISSING_CCC_COMPONENTS': 'مكونات CCC مفقودة',
                'IMPOSSIBLE_VALUE': 'قيمة مستحيلة',
                'UI_FALLBACK_LOWER_RELIABILITY': 'مسار UI بديل',
                'MISSING_FROM_EXCEL': 'غير موجود في إكسل',
                'UI_PROXY_INTERPOLATED': 'استيفاء UI',
                'UI_PROXY_CARRY': 'ترحيل UI',
                'UI_PROXY_DEFAULT': 'افتراضي UI',
                'YEAR_OUTSIDE_SOURCE': 'خارج نطاق المصدر',
            }.get(reason, reason)
            conf_raw = c.get('confidence_score')
            if conf_raw is None:
                conf_raw = c.get('reliability')
            try:
                conf = int(round(float(conf_raw))) if conf_raw is not None else None
            except Exception:
                conf = None
            conf_text = f" · {conf}%" if isinstance(conf, int) else ""
            ...
            if status and status not in ('COMPUTED',):
                return f"{disp} ??{conf_text}"
            return f"{disp}{conf_text}"

        def _lookup_numeric_ratio(year, ratio_key):
            ...
            if isinstance(raw_v_num, (int, float)):
                return float(raw_v_num)
            # try UI fallback (layered) if direct ratio missing
            fb = _safe_display_fallback(year, ratio_key)
            if isinstance(fb, (int, float)):
                return float(fb)
            return None

        def _proxy_fill_missing(year, ratio_key):
            """
            Missing values are allowed in ratio display.
            No interpolation, carry, or default fabrication is permitted here.
            """
            return None, None, 0
        ...
        def _safe_display_fallback(year, ratio_key):
            ...
            # Fallback: derive market cap from price * shares if missing
            if market_cap is None:
                _price = _n(l2.get('market:price') or l2.get('yahoo:price') or l4.get('yahoo:price'))
                _shares = _n(
                    l2.get('market:shares_outstanding')
                    or l2.get('yahoo:shares_outstanding')
                ...
                if _price not in (None, 0) and _shares not in (None, 0):
                    market_cap = float(_price) * float(_shares)
            ...
            # Fallback: derive total debt from balance sheet if missing
            if total_debt is None:
                lt_debt = _n(
                    row.get('LongTermDebtNoncurrent')
                    or row.get('LongTermDebt')
                ...
                if lt_debt is not None or cur_debt is not None:
                    total_debt = float(lt_debt or 0.0) + float(cur_debt or 0.0)
            ...
            if ratio_key == 'ev_ebitda':
                ...
                if ebitda is None:
                    ...
                    if op is not None:
                        ...
                        # last-resort proxy: EBITDA ~= Operating Income
                        ebitda = float(op)
        ...
        def insert_ratio(display_name, ratio_key, explanation=''):
            ...
            for y in years:
                if y not in data_by_year:
                    c = {
                        ...
                        'reason': 'YEAR_OUTSIDE_SOURCE',
                        'reliability': 0,
                        'source': 'sec_missing_year',
                    }
                ...
                if ui_excel_lock:
                    ...
                    has_metric = (not ui_lock_ratio_metrics) or (str(ratio_key).strip().lower() in ui_lock_ratio_metrics)
                    raw_v = rr.get(ratio_key) if has_metric else None
                    ...
                    if isinstance(raw_v_num, (int, float)):
                        ...
                        row.append(f"— ({reason}) ?? · 0%" if reason else "— ?? · 0%")
                        contracts_by_year[y] = {
                            'value': None,
                            'status': 'NOT_COMPUTABLE',
                    ...
                    contracts_by_year[y] = c
                    row.append(fmt_ratio_contract(ratio_key, c))
                    if isinstance((c or {}).get('value') if isinstance(c, dict) else None, (int, float)):
                        any_computed = True
                    else:
                        any_missing = True
            ...
            if not any_computed or any_missing:
                tags.append('missing_row')
        ...
        self.ratios_tree.tag_configure('header', background=PALETTE['surface_alt'], foreground=PALETTE['text'], font=FONTS['label'])
        self.ratios_tree.tag_configure('child_row', font=FONTS['tree'])
        self.ratios_tree.tag_configure('zebra_even', background=PALETTE['surface_soft'], foreground=PALETTE['text'])
        self.ratios_tree.tag_configure('zebra_odd', background=PALETTE['surface'], foreground=PALETTE['text'])
        self.ratios_tree.tag_configure('fallback_row', foreground='#b35a00')
        self.ratios_tree.tag_configure('missing_row', foreground='#c0392b')
        ...
        def insert_ratio_group(title, entries):
            visible_entries = []
            present_keys = {e[1] for e in entries}
            for entry in entries:
                rkey = entry[1]
                # Skip duplicate "used" variants if base ratio exists.
                if rkey.endswith('_used'):
                    base_key = rkey[:-5]
                    if base_key in present_keys:
                        continue
        ...
        if years:
            summary_by_lang = {
                'ar': (
                    f"عرض شامل لنسب الشركة {name} — الفترة: {years[0]} - {years[-1]}\n"
                    "تم تجميع النسب في فئات رئيسية لتسهيل التحليل والمقارنة."
                ),
                'en': (
    ...
    def _metric_meaning_text(self, metric_key: str, value, strategic: bool = False) -> str:
        ...
        fallback_meaning = {
            'ev_ebitda': {
                'ar': 'القيمة الأقل في EV/EBITDA (ضمن نطاق معقول) قد تشير عادةً إلى تقييم أرخص نسبيًا.',
            ...
            'wacc': {
                'ar': 'متوسط تكلفة التمويل، والقيم الأقل تدعم التقييم عادةً.',
        ...
        return {'ar': 'تفسير تحليلي معياري.', 'en': 'Standard analytical interpretation.', 'fr': 'Interprétation analytique standard.'}.get(lang, 'Standard analytical interpretation.')
    ...
    def _on_ratio_tree_click(self, event):
        ...
        reason_maps = {
            'ar': {
                'MISSING_SEC_CONCEPT': 'مفهوم SEC المطلوب غير متوفر لهذه السنة.',
                'MISSING_MARKET_DATA': 'مدخلات السوق المطلوبة غير متوفرة (السعر/القيمة السوقية/بيتا).',
                'MISSING_MARKET_LAYER': 'طبقة بيانات السوق غير مفعلة.',
                'MISSING_REQUIRED_LAYER': 'إحدى الطبقات المطلوبة غير متاحة أو معطلة.',
                'PERIOD_MISMATCH': 'فترات البيانات غير متطابقة.',
                'UNIT_MISMATCH': 'وحدات البيانات غير متوافقة.',
                'ZERO_DENOMINATOR': 'المقام صفر؛ القسمة غير ممكنة.',
                'INSUFFICIENT_HISTORY': 'السجل التاريخي غير كافٍ لهذه النسبة.',
                'DATA_NOT_APPLICABLE': 'النسبة غير منطبقة على البيانات المتاحة.',
                'NOT_APPLICABLE_FOR_SECTOR': 'النسبة مخفية لهذا القطاع.',
                'UI_FALLBACK_LOWER_RELIABILITY': 'تم الحساب عبر مسار بديل بموثوقية أقل من المسار الأساسي.',
                'NOT_COMPUTABLE': 'تعذر حساب النسبة من المدخلات الحالية.',
                'UNKNOWN': 'سبب غير محدد من محرك النسب.',
            },
            'en': {
        ...
        cell_upper = str(cell_text or '').strip().upper()
        cell_not_computable = (cell_upper.startswith('N/A') or cell_upper in {'—', '-', ''})
        status = str(contract.get('status') or ('NOT_COMPUTABLE' if cell_not_computable else 'COMPUTED'))
        value = contract.get('value')
        ...
        labels = {
            'ar': (
                ...
                'الثقة', 'المعادلة', 'المصدر', 'الفترة', 'المدخلات المستخدمة',
                'القيم الخام المستخدمة', 'المدخلات الناقصة', 'السبب الفني', 'المعنى', 'تفاصيل النسبة'
            ),
            'en': (
    ...
    def _compute_per_year_metrics(self, data_by_year, ratios_by_year):
            ...
            if isinstance(rule, list):
                ...
                for action in rule:
                    try:
                        ...
                        continue
                    if split_factor > 1.0 and int(year_key) < effective_year:
                        factor *= split_factor
                if factor > 1.0:
                    return factor
            return sf if isinstance(sf, (int, float)) and sf > 1.0 else 1.0
        ...
        for idx, y in enumerate(years):
            ...
            def get_roe(net_income, total_equity, ratios_roe=None):
                # Prefer ratio sheet when available and plausible.
                if ratios_roe not in (None, False):
                    try:
                        v = float(ratios_roe)
                        if abs(v) < 5.0:
                            return {
                        ...
                        pass
                # Fallback to accounting definition: NI / Total Equity.
                if total_equity not in (None, 0):
                    try:
                        eq = float(total_equity)
                        if abs(eq) > 1:
                            roe_v = float(net_income) / eq
                            flag = "EXTREME_ROE_BUYBACK" if abs(roe_v) > 2.0 else None
                            return {
                                "value": roe_v,
                                "reliability": 80 if not flag else 60,
                                "flag": flag,
                        ...
                        pass
                return {"value": None, "reason": "MISSING_EQUITY_DATA"}

            def get_ccc(inventory_days, ar_days, ap_days, revenue_inferred=False):
                if all(v is not None for v in [inventory_days, ar_days, ap_days]):
                    try:
                        ccc_v = float(inventory_days) + float(ar_days) - float(ap_days)
                        return {
                            "value": ccc_v,
                            "reliability": 70 if revenue_inferred else 90,
                            "source": "component_sum",
                            "display": f"{ccc_v:.1f} ???",
                        }
                    except Exception:
                        pass
                return {"value": None, "reason": "missing_ccc_components"}

            vals['ROIC'] = get_ratio('roic')
            vals['ROA'] = get_ratio('roa')
            equity_total = first_not_none(
            ...
            ni_for_roe = first_not_none(get_data('NetIncomeLoss'), get_data('Net Income'), get_data('ProfitLoss'))
            vals['ROE'] = get_roe(
                net_income=ni_for_roe if ni_for_roe is not None else 0.0,
                total_equity=equity_total,
                ratios_roe=get_ratio('roe'),
            )
            ...
            try:
                ...
                if prev_y:
                    ...
                    rev = (
                        get_data('Revenues')
                        or get_data('SalesRevenueNet')
                        or get_data('TotalRevenue')
                        or get_data('TotalNetRevenues')
                        or get_data('NetRevenues')
                        or get_data('NonInterestRevenue')
                        or 0.0
                    )
                    rev_prev = (
                        get_data_year(prev_y, 'Revenues')
                        or get_data_year(prev_y, 'SalesRevenueNet')
                        or get_data_year(prev_y, 'TotalRevenue')
                        or get_data_year(prev_y, 'TotalNetRevenues')
                        or get_data_year(prev_y, 'NetRevenues')
                        or get_data_year(prev_y, 'NonInterestRevenue')
                    )
            ...
            vals['AR_Days'] = dso
            vals['AP_Days'] = dpo
            ccc_contract = get_contract('ccc_days')
            ccc_val = ccc_contract.get('value')
            if ccc_val is None:
                rev_now = first_not_none(
                    get_data('Revenues'),
                    get_data('Revenue'),
                    get_data('SalesRevenueNet'),
                    get_data('RevenueFromContractWithCustomerExcludingAssessedTax'),
                    get_data('TotalRevenue'),
                    get_data('NetRevenues'),
                )
                vals['CCC_Days'] = get_ccc(
                    inventory_days=dih,
                    ar_days=dso.get('value') if isinstance(dso, dict) else dso,
                    ap_days=dpo,
                    revenue_inferred=(rev_now is None),
                )
            else:
                vals['CCC_Days'] = ccc_contract
            wacc_bank_low = False
    ...
    def display_strategic_analysis(self):
        ...
        year_col_w = 90 if len(years) > 12 else 130
        for c in cols:
            self.strat_tree.heading(c, text=self._translate_financial_item(c))
            if c == metric_col:
                self.strat_tree.column(c, width=320, anchor='w', stretch=True)
            else:
                self.strat_tree.column(c, width=year_col_w, anchor='center', stretch=False)
        self.strat_tree.tag_configure('header', background=PALETTE['surface_alt'], foreground=PALETTE['text'], font=FONTS['label'])
        self.strat_tree.tag_configure('child_row', font=FONTS['tree'])
        self.strat_tree.tag_configure('zebra_even', background=PALETTE['surface_soft'], foreground=PALETTE['text'])
        self.strat_tree.tag_configure('zebra_odd', background=PALETTE['surface'], foreground=PALETTE['text'])
        ...
        def format_contract(v):
            ...
            base = f"{base} · {rel}%" if isinstance(rel, int) else base
        ...
        def insert_metric(display, key, fmt='num'):
            ...
            for y in years:
                ...
                if c is not None:
                    row.append(c)
                    if isinstance((v or {}).get('value') if isinstance(v, dict) else None, (int, float, str)):
                        # Non-empty contract payload (including text metrics) counts as present.
                        vv = (v or {}).get('value') if isinstance(v, dict) else None
                        if vv not in (None, '', 'N/A'):
                            any_computed = True
    ...
    def _apply_pre_export_quality_gate(self, years, data_by_year, ratios_by_year, data_layers):
                ...
                if isinstance(rule, list):
                    ...
                    for action in rule:
                        try:
                            ...
                            continue
                        if split_factor > 1.0 and year < effective_year:
                            factor *= split_factor
                    if factor > 1.0:
                        return factor
                else:
            ...
            if anchor_year is not None and anchor_debt is not None and prefix_val > (1.8 * anchor_debt):
                ...
                if ratio_anchor is not None and 0.02 <= ratio_anchor <= 1.5:
                    ...
                    for py in prefix_years:
                        ...
                        rr = ratios_by_year.setdefault(py, {})
        ...
        def _interpolate_missing_ratio(metric_key, year_key):
            """
            Linear interpolation when a missing year is surrounded by known years.
            Returns None when interpolation is not feasible.
            """
            try:
                year_list = sorted(int(yy) for yy in years if str(yy).isdigit())
                if year_key not in year_list:
                    return None
                i = year_list.index(year_key)
                if i <= 0 or i >= len(year_list) - 1:
                    return None
                prev_y = next_y = None
                prev_v = next_v = None
                for j in range(i - 1, -1, -1):
                    py = year_list[j]
                    pv = _num((ratios_by_year.get(py, {}) or {}).get(metric_key))
                    if pv is not None:
                        prev_y, prev_v = py, float(pv)
                        break
                for j in range(i + 1, len(year_list)):
                    ny = year_list[j]
                    nv = _num((ratios_by_year.get(ny, {}) or {}).get(metric_key))
                    if nv is not None:
                        next_y, next_v = ny, float(nv)
                        break
                if prev_y is None or next_y is None or next_y == prev_y:
                    return None
                w = (float(year_key) - float(prev_y)) / float(next_y - prev_y)
                est = prev_v + (w * (next_v - prev_v))
                return {
                    "value": est,
                    "status": "INTERPOLATED_ESTIMATE",
                    "reliability": 60,
                    "reason": f"Interpolated between {prev_y} and {next_y}",
                }
            except Exception:
                return None
        ...
        for idx, y in enumerate(years):
            ...
            if assets_chk is None or liab_chk is None or eq_chk is None:
                ...
                issues.append(f"{y}: Balance anchors missing ({', '.join(missing)}) - balance-based ratios may stay —")
            ...
            # Interpolate mandatory margins when a single year is structurally missing and
            # surrounded by valid years (e.g., NVDA 2019 style gap).
            for _mk in ('gross_margin', 'operating_margin', 'net_margin'):
                if _num(r.get(_mk)) is None:
                    interp = _interpolate_missing_ratio(_mk, y)
                    if interp:
                        r[_mk] = interp.get('value')
                        r[f'{_mk}_status'] = interp.get('status')
                        r[f'{_mk}_reliability'] = interp.get('reliability')
                        r[f'{_mk}_reason'] = interp.get('reason')
                        r[f'{_mk}_source'] = 'QUALITY_GATE_INTERPOLATED_ESTIMATE'
            ...
            split_factor_y = _split_ratio_for_year(y)
            eps_for_pe = eps_now
            if price_y not in (None, 0) and eps_now not in (None, 0):
                pe_raw = price_y / eps_now
                if split_factor_y > 1.0 and abs(pe_raw) < 2.0:
                    eps_split_adj = eps_now / split_factor_y
                    pe_split_adj = price_y / eps_split_adj if eps_split_adj not in (None, 0) else None
                    if pe_split_adj not in (None, 0) and 2.0 <= abs(pe_split_adj) <= 1_500:
                        eps_for_pe = eps_split_adj
                        if abs(eps_now - eps_split_adj) > 1e-12:
                            r['eps_basic'] = eps_split_adj
                            r['eps_source'] = f'QUALITY_GATE_SPLIT_ADJUSTED_X{split_factor_y:g}'
                            issues.append(f"{y}: eps_basic split-adjusted for valuation consistency ({eps_now} -> {eps_split_adj})")
                pe_annual = price_y / eps_for_pe
            ...
            if price_y not in (None, 0) and bvps_now not in (None, 0):
                ...
                if split_factor_y > 1.0 and abs(pb_raw) < 0.10:
                    bvps_split_adj = bvps_now / split_factor_y
                    pb_split_adj = price_y / bvps_split_adj if bvps_split_adj not in (None, 0) else None
                    if pb_split_adj not in (None, 0) and 0.10 <= abs(pb_split_adj) <= 500.0:
                        bvps_for_pb = bvps_split_adj
                        if abs(bvps_now - bvps_split_adj) > 1e-12:
                            r['book_value_per_share'] = bvps_split_adj
                            r['book_value_per_share_source'] = f'QUALITY_GATE_SPLIT_ADJUSTED_X{split_factor_y:g}'
                            issues.append(f"{y}: book_value_per_share split-adjusted for valuation consistency ({bvps_now} -> {bvps_split_adj})")
                pb_annual = price_y / bvps_for_pb
                ...
                if split_factor_y > 1.0 and abs(pb_annual) < 1.0 and mcap not in (None, 0) and equity not in (None, 0):
                    pb_candidates = [
                        mcap / equity,
                        mcap / (equity * 1_000.0),
                        mcap / (equity * 1_000_000.0),
                    ]
                    pb_plausible = [c for c in pb_candidates if 0.10 <= c <= 200.0]
                    if pb_plausible:
                        pb_rebuilt = min(pb_plausible, key=lambda c: abs(c - 8.0))
                        r['pb_ratio'] = pb_rebuilt
                        r['pb_ratio_source'] = 'SPLIT_AWARE_MARKETCAP_EQUITY_OVERRIDE'
                        r['pb_ratio_used'] = pb_rebuilt
                        r['pb_ratio_used_source'] = 'SPLIT_AWARE_MARKETCAP_EQUITY_OVERRIDE'
                        if price_y not in (None, 0) and pb_rebuilt not in (None, 0):
                            rebuilt_bvps = price_y / pb_rebuilt
                            if rebuilt_bvps not in (None, 0):
                                r['book_value_per_share'] = rebuilt_bvps
                                r['book_value_per_share_source'] = 'SPLIT_AWARE_MARKETCAP_EQUITY_OVERRIDE'
                        issues.append(f"{y}: pb_ratio rebuilt from market_cap/equity under split-aware regime ({pb_annual} -> {pb_rebuilt})")
            ...
            pb_now = _num(r.get('pb_ratio'))
            if m_pb is not None and m_pb > 0 and (pb_now is None or abs(pb_now) < 0.10):
                r['pb_ratio'] = m_pb
                r['pb_ratio_source'] = 'MARKET_TTM_OVERRIDE_IMPLAUSIBLE_ANNUAL'
                r['pb_ratio_used'] = m_pb
                r['pb_ratio_used_source'] = 'MARKET_TTM_OVERRIDE_IMPLAUSIBLE_ANNUAL'
                issues.append(f"{y}: pb_ratio restored from market_ttm due implausible annual value ({pb_now} -> {m_pb})")
        ...
        for y in years:
            ...
            if price_y not in (None, 0) and eps_y not in (None, 0):
                ...
                split_factor_lock = _split_ratio_for_year(y)
                existing_pe = _num(r.get('pe_ratio'))
                if split_factor_lock > 1.0 and abs(pe_annual) < 2.0 and existing_pe not in (None, 0) and abs(existing_pe) >= 2.0:
                    pe_annual = existing_pe
                if 0.1 <= abs(pe_annual) <= 1_500:
            ...
            if price_y not in (None, 0) and bvps_y not in (None, 0):
                ...
                existing_pb = _num(r.get('pb_ratio'))
                split_factor_lock = _split_ratio_for_year(y)
                if split_factor_lock > 1.0 and abs(pb_annual) < 1.0 and existing_pb not in (None, 0) and abs(existing_pb) >= 1.0:
                    pb_annual = existing_pb
                if 0.05 <= abs(pb_annual) <= 500:
    ...
    def _snapshot_tree_df(self, tree_attr_name):
        try:
            import pandas as pd
        except Exception:
            return None
    ...
    def _snapshot_ratios_tree_df(self):
        return self._snapshot_tree_df('ratios_tree')

    def _snapshot_strategic_tree_df(self):
        return self._snapshot_tree_df('strat_tree')
    ...
    def _year_dict_label_quality(self, year_dict):
        labels = []
        for _y, row in (year_dict or {}).items():
            if isinstance(row, dict):
                for key in row.keys():
                    lbl = str(key or '').strip()
                    if lbl:
                        labels.append(lbl)
        if not labels:
            return {'count': 0, 'weak': 0, 'weak_ratio': 1.0}
        weak = sum(1 for lbl in labels if self._label_looks_truncated_or_weak(lbl))
        return {'count': len(labels), 'weak': weak, 'weak_ratio': weak / max(1, len(labels))}
    ...
    def load_results_from_excel(self, file_path=None, show_success_message=True):
        ...
        try:
            ...
            data_by_year = self._sheet_to_year_dict(raw_df, ['Concept', 'Line Item', 'Item', 'سنة'], allow_text=False)
            layer1_by_year = self._sheet_to_year_dict(layer1_df, ['Raw Label', 'Line Item', 'Concept', 'Item', 'سنة'], allow_text=False)
            for stmt_df in (income_df, balance_df, cashflow_df):
                stmt_rows = self._sheet_to_year_dict(stmt_df, ['Line Item', 'Item', 'Concept', 'سنة'], allow_text=False)
            ...
            # If Layer1 labels are truncated (e.g., Ac/Tc/Cu), prefer merged statement labels.
            try:
                l1q = self._year_dict_label_quality(layer1_by_year)
                dq = self._year_dict_label_quality(data_by_year)
                if (
                    l1q.get('count', 0) > 0
                    and l1q.get('weak_ratio', 1.0) >= 0.20
                    and dq.get('weak_ratio', 1.0) < l1q.get('weak_ratio', 1.0)
                ):
                    layer1_by_year = dict(data_by_year)
            except Exception:
                pass

            ratios_by_year = self._sheet_to_year_dict(ratios_df, ['Metric', 'Ratio', 'Name', 'الوحدة'], allow_text=False)
            strategic_by_year = self._sheet_to_year_dict(strategic_df, ['Metric', 'Indicator', 'المؤشر'], allow_text=True)
            ...
            try:
                ...
                imported_ratio_metrics = set()
                imported_strategic_metrics = set()
            layer2_by_year = self._sheet_to_year_dict(layer2_df, ['Normalized Label', 'Item', 'الوحدة'], allow_text=True)
            layer4_by_year = self._sheet_to_year_dict(layer4_df, ['Normalized Label', 'Item', 'الوحدة'], allow_text=True)
            ...
            loaded = {
                ...
                'institutional_outputs': {
                    'classification': {
                        'primary_profile': effective_sub_profile,
                        'confidence': imported_meta.get('confidence'),
                    },
                    'classifier_diagnostics': {
                        'sector': effective_sub_profile,
            ...
            gate_issues = self._apply_pre_export_quality_gate(
                gate_years,
                loaded.get('data_by_year', {}),
                loaded.get('financial_ratios', {}),
                loaded.get('data_layers', {}),
            )
            if gate_issues:
                loaded['load_quality_gate_issues'] = gate_issues
                ...
                for y in (years or []):
                    ...
                    if status == 'COMPUTED' and isinstance(val, (int, float)):
                        try:
                            ...
                            disp = str(val)
                        rel_txt = f"{int(rel)}%" if isinstance(rel, (int, float)) else '??? ?????'
                        explanation_txt = str(explanation or '').strip()
                        investor_view = _classify_ratio_signal(ratio_key, val)
                        narrative_parts = [f"?????? ???????? {disp}."]
                        if explanation_txt:
                            narrative_parts.append(explanation_txt)
                        narrative_parts.append(investor_view)
                        narrative_parts.append(f"?????? ???????: {source}.")
                        narrative_parts.append(f"???? ?????????: {rel_txt}.")
                        narrative = ' '.join(narrative_parts)
                    else:
                        missing_txt = ', '.join(str(m) for m in missing) if missing else '??? ????'
                        if (reason or status) == 'DATA_NOT_APPLICABLE':
                            narrative = f"?????? ??? ????? ??????? ?? ??? ????? ?? ??? ??????? ???? ?? ??????. ??????: {source}."
                        else:
                            narrative = (
                                f"???? ?????? ??????. ?????: {reason or 'UNKNOWN'}. "
                                f"???????? ??????? ?? ???????: {missing_txt}. ??????: {source}."
                            )
            ...
            for _, r in forecasts_df.iterrows():
                ...
                for metric_name, base_key, bull_key, bear_key, conf, method in [
                    ...
                    try:
                        ...
                        spread_txt = '??? ????'
                    conf_txt = f"{float(conf):.1f}%" if isinstance(conf, (int, float)) else '??? ????'

                    def _scenario_narrative(label, value):
                        if label == 'Base':
                            if isinstance(value, (int, float)):
                                return (
                                    f"????????? ??????? ?? {metric_name} ?? ??? {y} ????? ?????? ??? {value:,.0f} "
                                    f"???? {conf_txt} ???????? {method or '??? ?????'}. "
                                    f"????? ?????? ??? ???????? ???????? ???? ??????? {spread_txt} ?? ????? ??????."
                                )
                            return f"????????? ??????? ?? {metric_name} ?? ??? {y} ??? ???? ????? ????? ???????."
                        if label == 'Bull':
                            if isinstance(value, (int, float)):
                                return (
                                    f"????????? ???????? ????? ?????? ???? ?? ?????? ?? {metric_name} ??? {value:,.0f}? "
                                    f"?????? ??? ???? ???????? ??????? ???????? ???? ?? ???????."
                                )
    ...
    @staticmethod
    def _verdict_context_rules(sub_sector: str) -> dict:
        sector = str(sub_sector or '').strip().lower()
        rules = {
            "hardware_platform": {
    ...
    EXTENDED_PEER_UNIVERSE = {
        ...
        "commercial_bank": ["JPM", "BAC", "WFC", "C", "USB", "PNC", "TFC"],
        "investment_bank": ["MS", "GS", "JPM", "BAC", "C"],
        "insurance_life": ["PRU", "MET", "AIG", "LNC", "AFL"],
        "insurance_broker": ["AON", "MMC", "AJG", "WTW", "BRO"],
        "insurance_pc": ["TRV", "CB", "PGR", "ALL", "HIG"],
    }

    def _get_peers(self, ticker: str, sub_sector: str, available_tickers):
        extended = self.EXTENDED_PEER_UNIVERSE.get(str(sub_sector or '').strip().lower(), [])
        available = {str(t).upper() for t in (available_tickers or [])}
        base = [str(t).upper() for t in extended if str(t).upper() != str(ticker or '').upper()]
        local_peers = [t for t in base if t in available]
        reference_peers = [t for t in base if t not in available]
        return {
            "local": local_peers,
        ...
        try:
            ...
            for y in years:
                ...
                if year_critical > 0:
                    ...
                    cautions.append(f'{year_critical} إشارات حرجة هذا العام.')
                if year_high > 0:
                    score -= min(16.0, 6.0 + (year_high * 3.0))
                    cautions.append(f'{year_high} إشارات عالية الأهمية هذا العام.')

                year_issues = issue_map.get(y, [])
                if year_issues:
                    issue_penalty_per_item = 2.5
                    if sector_norm in {'consumer_staples'}:
                        issue_penalty_per_item = 1.2
                    score -= min(16.0, float(len(year_issues)) * issue_penalty_per_item)
                    cautions.append(f'{len(year_issues)} ملاحظات في هذا العام تحتاج متابعة.')
                ...
                if any(k in issue_txt for k in hard_markers):
                    score -= 6.0
                    cautions.append('تم استخدام مسار تصحيح/احتياط مع موثوقية أقل.')
                ...
                if sector_rules.get('ignore_low_current_ratio'):
                ...
                if pe is None:
                    score -= 3.0
                    cautions.append('مكرر الربحية غير متاح.')
                elif pe > 35:
                    if sector_norm in {'software_saas', 'semiconductor_fabless'}:
                        score -= 5.0
                        cautions.append('مكرر الربحية مرتفع جدًا مقارنة بالقطاع.')
                    else:
                        score -= 8.0
                        cautions.append('مكرر الربحية مرتفع مقارنة بالنطاق المعتاد.')
                elif pe > 25:
                    score -= 4.0
                    cautions.append('مكرر الربحية أعلى من المعتاد.')
                if pb is None:
                    score -= 2.5
                    cautions.append('مكرر القيمة الدفترية غير متاح.')
                elif pb < 0 and sector_norm in {'software_saas', 'hardware_platform', 'consumer_staples', 'insurance_broker'}:
                    cautions.append('مكرر القيمة الدفترية السالب قد يشير إلى حقوق ملكية سالبة أو بنية رأس مال خاصة.')
                elif pb > float(sector_rules.get('pb_penalty_threshold', 40.0)):
                    score -= 8.0
                    cautions.append('مكرر القيمة الدفترية مرتفع جدًا مقارنة بالنطاق المعتاد.')
                elif pb > 20:
                    score -= 4.0
                    cautions.append('مكرر القيمة الدفترية مرتفع.')
                if requires_fcf and 'fcf_yield' not in blocked and fcfy is None:
                    score -= 3.0
                    cautions.append('عائد التدفق النقدي الحر غير متاح.')
                elif (
                    requires_fcf
                    and isinstance(fcfy, (int, float))
                    and fcfy < 0.03
                    and not (
                        sector_rules.get('ignore_low_fcf_yield')
                        or (sector_rules.get('ignore_low_fcf_yield_if_dividend') and isinstance(_num(rr.get('dividend_yield')), (int, float)) and _num(rr.get('dividend_yield')) > 0)
                    )
                ):
                    score -= 6.0
                    cautions.append('عائد التدفق النقدي الحر منخفض جدًا مقارنة بالنطاق المعتاد.')
                elif (
                    requires_fcf
                    and isinstance(fcfy, (int, float))
                    and fcfy < 0.05
                    ...
                    score -= 3.0
                    cautions.append('عائد التدفق النقدي الحر منخفض.')
                elif requires_fcf and isinstance(fcfy, (int, float)) and fcfy >= 0.07:
                    score += 3.0
                    positives.append('عائد التدفق النقدي الحر قوي ومطمئن.')
                if mcap is None:
                    score -= 2.0
                ...
                if uses_industrial_margins and isinstance(gross_margin, (int, float)):
                    if gross_margin >= 0.40:
                        score += 3.0
                        positives.append('هامش إجمالي قوي بالنسبة للقطاع.')
                    elif gross_margin < 0.20:
                        score -= 4.0
                        cautions.append('هامش الربح الإجمالي ضعيف.')
                if (uses_industrial_margins or sector_norm == 'insurance_broker') and isinstance(operating_margin, (int, float)):
                    if operating_margin >= 0.25:
                        score += 3.0
                        positives.append('هامش تشغيلي قوي.')
                    elif operating_margin < 0.08:
                        score -= 4.0
                        cautions.append('هامش تشغيلي ضعيف.')
                if isinstance(roic, (int, float)):
                    if roic >= 0.18:
                        score += 6.0
                        positives.append('العائد على رأس المال المستثمر قوي جدًا.')
                    elif roic >= 0.12:
                        score += 3.0
                        positives.append('العائد على رأس المال المستثمر جيد.')
                    elif roic < 0.06:
                        score -= 5.0
                        cautions.append('العائد على رأس المال المستثمر ضعيف.')
                if uses_liquidity and isinstance(current_ratio, (int, float)):
                    if current_ratio < 0.90:
                        score -= 6.0
                        cautions.append('السيولة قصيرة الأجل أقل من 1 ومصدر قلق.')
                    elif current_ratio < 1.0:
                        score -= 3.0
                        cautions.append('السيولة قصيرة الأجل دون 1 وتحتاج متابعة.')
                    elif current_ratio >= 1.2:
                        score += 1.0
                        positives.append('السيولة قصيرة الأجل جيدة.')
                if uses_liquidity and isinstance(quick_ratio, (int, float)):
                    if quick_ratio < 0.50:
                        score -= 4.0
                        cautions.append('النسبة السريعة ضعيفة.')
                    elif quick_ratio >= 0.80:
                        score += 1.5
                        positives.append('النسبة السريعة مطمئنة.')
                if isinstance(debt_to_equity, (int, float)):
                    debt_threshold = float(sector_rules.get('debt_penalty_threshold', 1.5))
                    if sector_rules.get('high_debt_normal'):
                        debt_threshold = max(debt_threshold, 2.5)
                    if sector_rules.get('ignore_pre_2017_debt') and y < 2017:
                        debt_threshold = 99.0
                    if debt_to_equity > debt_threshold:
                        score -= 5.0
                        cautions.append('الرفع المالي مرتفع.')
                    elif debt_to_equity > min(debt_threshold, 1.0):
                        score -= 2.5
                        cautions.append('الرفع المالي أعلى من المعتاد.')
                    elif debt_to_equity < 0.8:
                        score += 2.0
                        positives.append('الرفع المالي منخفض ومطمئن.')
                if isinstance(net_debt_ebitda, (int, float)):
                    if net_debt_ebitda < 0:
                        score += 3.0
                        positives.append('صافي الدين إلى EBITDA سلبي، وهو عامل مطمئن.')
                    elif net_debt_ebitda < 1.5:
                        score += 2.0
                        positives.append('صافي الدين إلى EBITDA جيد.')
                ...
                if isinstance(interest_coverage, (int, float)):
                    if interest_coverage >= 10:
                        score += 4.0
                        positives.append('تغطية الفوائد ممتازة.')
                    elif interest_coverage >= 5:
                        score += 1.5
                        positives.append('تغطية الفوائد جيدة.')
                    elif interest_coverage < 3:
                        score -= 7.0
                        cautions.append('تغطية الفوائد ضعيفة.')
                ...
                if uses_combined:
                    ...
                    if isinstance(combined, (int, float)):
                        if combined < 0.98:
                            score += 2.0
                            positives.append('نسبة الخسارة/الدمج جيدة.')
                        elif combined > 1.08:
                            score -= 5.0
                            cautions.append('نسبة الخسارة/الدمج مرتفعة.')
                ...
                if isinstance(rev_conf, (int, float)) and isinstance(ni_conf, (int, float)):
                    ...
                    if c < 50:
                        score -= 10.0
                        cautions.append('ثقة التوقعات منخفضة.')
                    elif c < 60:
                        score -= 5.0
                        cautions.append('ثقة التوقعات متوسطة إلى منخفضة.')
                    elif c >= 75:
                        score += 1.0
                        positives.append('ثقة التوقعات جيدة.')
                ...
                roic_override_threshold = sector_rules.get('roic_override_threshold')
                if verdict == 'FAIL' and isinstance(roic, (int, float)) and isinstance(roic_override_threshold, (int, float)):
                    if float(roic) > float(roic_override_threshold):
                        verdict = 'WATCH'
                        score = max(score, 65.0)
                        positives.append('تم رفع الحكم بسبب ROIC قوي جدًا.')

                reasons = positives[:3] + cautions[:4]
                if not reasons:
                    reasons = ['لا توجد أسباب نوعية إضافية بعد التقييم الحالي.']
            ...
            if global_issues:
                ...
                rows.append({
                    ...
                    'Reasons': 'لا توجد ملاحظات رئيسية إضافية على المستوى العام.',
                    'Positive_Signals': '',
                    'Caution_Signals': '',
    ...
    def _build_financial_analysis_system_export_frames(self):
        try:
            import pandas as pd
            fas = (self.current_data.get('financial_analysis_system', {}) if self.current_data else {}) or {}
            sector_norm = self._normalize_sector_for_packs(self._get_sector_profile())
            if not isinstance(fas, dict) or not fas:
                ...
                score_df = pd.DataFrame([{'Metric': 'score', 'Value': None}])
                integrity_df = pd.DataFrame([{'Metric': 'coverage_pct', 'Value': None}])
                return summary_df, kpis_df, score_df, integrity_df

            model = fas.get('model') or {}
            score = fas.get('score') or {}
            sig = fas.get('financial_signature') or {}
            integ = fas.get('data_integrity') or {}
            integ_summary = (integ.get('summary') or {}) if isinstance(integ, dict) else {}
            kpis_payload = fas.get('kpis') or {}
            if isinstance(kpis_payload, dict) and isinstance(kpis_payload.get('kpis'), dict):
                kpis = kpis_payload.get('kpis') or {}
            elif isinstance(kpis_payload, dict):
                kpis = kpis_payload
            else:
                kpis = {}

            summary_rows = [
                {'Metric': 'ticker', 'Value': fas.get('ticker')},
                {'Metric': 'model', 'Value': model.get('model') if isinstance(model, dict) else None},
                {'Metric': 'model_confidence', 'Value': model.get('confidence') if isinstance(model, dict) else None},
                {'Metric': 'score', 'Value': score.get('score') if isinstance(score, dict) else None},
                {'Metric': 'score_status', 'Value': score.get('status') if isinstance(score, dict) else None},
                {'Metric': 'signature_overall', 'Value': sig.get('overall_signature_score') if isinstance(sig, dict) else None},
                {'Metric': 'growth_profile', 'Value': sig.get('growth_profile') if isinstance(sig, dict) else None},
                {'Metric': 'profitability', 'Value': sig.get('profitability') if isinstance(sig, dict) else None},
                {'Metric': 'capital_intensity', 'Value': sig.get('capital_intensity') if isinstance(sig, dict) else None},
                {'Metric': 'financial_risk', 'Value': sig.get('financial_risk') if isinstance(sig, dict) else None},
            ]
            summary_df = pd.DataFrame(summary_rows)

            kpi_rows = []
            for bucket in ('primary', 'secondary', 'ignored'):
                for k in (kpis.get(bucket) or []):
                    kpi_rows.append({'Bucket': bucket, 'KPI': k})
            kpis_df = pd.DataFrame(kpi_rows if kpi_rows else [{'Bucket': 'primary', 'KPI': None}])

            score_details_rows = []
            details = (score.get('details') or []) if isinstance(score, dict) else []
            for d in details:
                score_details_rows.append({
                    'Metric': d.get('metric'),
                    'Value': d.get('value'),
                    'Metric_Score': d.get('metric_score'),
                    'Weight': d.get('weight'),
                    'Contribution': d.get('contribution'),
                })
            if not score_details_rows:
                score_details_rows = [{'Metric': 'details', 'Value': None, 'Metric_Score': None, 'Weight': None, 'Contribution': None}]
            score_df = pd.DataFrame(score_details_rows)

            integrity_rows = []
            for k, v in integ_summary.items():
                integrity_rows.append({'Metric': k, 'Value': v})
            ...
            import pandas as pd
            summary_df = pd.DataFrame([{'Metric': 'FAS_Status', 'Value': f'ERROR: {e}'}])
            return summary_df, pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    def export_to_excel(self):
        ...
        if not self.current_data:
            ...
            if not self.current_data:
                messagebox.showwarning(self._t('msg_warning'), self._translate_ui_text("?? ???? ?????? ????? ???????. ??? ???????? ?????."))
                return
        try:
            import pandas as pd
        except Exception as e:
            messagebox.showerror(self._t('msg_error'), f"{self._translate_ui_text('????? ????? pandas ? openpyxl')}:\n{e}\n\npip install pandas openpyxl")
            return
        ...
        per_year = self._compute_per_year_metrics(data_by_year, ratios_by_year)
        ratios_by_year, per_year = self._sync_strategic_ratio_maps(years, ratios_by_year, per_year)
        # Apply UI proxy fill so Excel matches on-screen values and avoids visible N/A.
        sector_profile_export = self._get_sector_profile()
        self._apply_ui_proxy_fill(years, ratios_by_year, per_year, sector_profile=sector_profile_export)
        if gate_issues:
            print(f"??? Quality gate applied {len(gate_issues)} corrections before export.")
        ...
        # Build an "Inputs_View" sheet that mirrors the UI inputs panel.
        try:
            saved_mode = getattr(self, '_sec_view_mode_var', tk.StringVar(value='official')).get()
            self._sec_view_mode_var.set('inputs')
            self.display_raw_data()
            ui_inputs_df = self._snapshot_raw_tree_df()
            if ui_inputs_df is not None and not ui_inputs_df.empty:
                inputs_df = ui_inputs_df.copy()
        except Exception:
            inputs_df = pd.DataFrame()
        finally:
            try:
        ...
        try:
            self.display_ratios()
            ui_ratios_df = self._snapshot_ratios_tree_df()
            if ui_ratios_df is not None and not ui_ratios_df.empty:
                ratios_df = ui_ratios_df.copy()
        except Exception:
            pass

        try:
            self.display_strategic_analysis()
            ui_strat_df = self._snapshot_strategic_tree_df()
            if ui_strat_df is not None and not ui_strat_df.empty:
                strat_df = ui_strat_df.copy()
        except Exception:
            pass
        ...
        fas_summary_df, fas_kpis_df, fas_score_df, fas_integrity_df = self._build_financial_analysis_system_export_frames()
        chart_data_by_year = {}
        try:
        ...
        try:
            ...
            with pd.ExcelWriter(fn, engine='openpyxl') as writer:
                ...
                fas_summary_df.to_excel(writer, sheet_name='FAS_Summary', index=False)
                fas_kpis_df.to_excel(writer, sheet_name='FAS_KPIs', index=False)
                fas_score_df.to_excel(writer, sheet_name='FAS_Score_Details', index=False)
                fas_integrity_df.to_excel(writer, sheet_name='FAS_Integrity', index=False)
            ...
            if verdict_val == 'APPROVED_FOR_EXPERT_REVIEW':
                ...
                messagebox.showwarning(
                    ...
                    f"Verdict: {verdict_val}\n"
                    f"Comparison Universe: {comparison_universe_count} | {comparison_universe_txt}\n"
                    f"???? ???????: Final_Acceptance ? Critical_Flags.\n"
                    f"UI Sync: {'OK' if reload_ok else 'FAILED'}\n"
                    f"Post-Export Audit: objects={audit_summary.get('object_like_cells', 0)} | normalized={audit_summary.get('normalized_cells', 0)}",
                )
        except Exception as e:
            ...
            messagebox.showerror(
                self._t('msg_error'),
                f"{self._translate_ui_text('??? ???????')}: {e}\n{self._translate_ui_text('???? ?????')}: outputs/export_error.log"
            )
    ...
    def _export_to_excel_minimal(self):
        ...
        try:
            ...
            messagebox.showerror(self._t('msg_error'), f"{self._translate_ui_text('??? ???????')}: {e}")
            return
        ...
        methodology_df = pd.DataFrame([
            ...
            {'Topic': 'Ratios', 'Rule': 'From ratio engine', 'Details': 'Unavailable values are rendered as — with traceability kept in audit sheets.'},
    ...
    def export_to_excel_safe(self):
        ...
        try:
            ...
            messagebox.showerror(
                ...
                f"{self._translate_ui_text('??? ??????? ??????')}:\n{e}\n\n"
                f"{self._translate_ui_text('?? ??? ????? ??? ????? ???? ???????? ?????? ??? ????? ???????? ?? ?????? ??????? ???????')}.\n"
                f"{self._translate_ui_text('???? ?????')}: outputs/export_error.log"
            )
    ...
    def display_forecasts(self):
        ...
        for c in cols:
            ...
            elif 'الإيرادات' in c or 'صافي الدخل' in c:
        ...
        for row in (bundle.get('rows') or []):
            ...
            self.forecast_tree.insert(
                ...
                values=(
                    ...
                    f"{float(rev_conf):.1f}%" if isinstance(rev_conf, (int, float)) else "—",
                    fmt(nv),
                    fmt(row.get('NetIncome_Bull')),
                    fmt(row.get('NetIncome_Bear')),
                    f"{float(ni_conf):.1f}%" if isinstance(ni_conf, (int, float)) else "—",
                    vol_txt,
                    model_q,
                    str(bundle.get('revenue', {}).get('method') or '—'),
                ),
            )
    ...
    def launch_professional_engine_v2(self):
        """Launches the new Streamlit-based professional engine in a separate process."""
        try:
            app_path = Path(__file__).parent / "financial_analyzer" / "ui" / "app.py"
            if not app_path.exists():
                messagebox.showerror(self._t('msg_error'), self._t('tool_v2_missing'))
                return
            subprocess.Popen(
            ...
            self._set_status_text("Professional V2 engine launched in separate window...")
        except Exception as e:
            messagebox.showerror(self._t('msg_error'), f"{self._t('tool_v2_launch_failed')}: {e}")
