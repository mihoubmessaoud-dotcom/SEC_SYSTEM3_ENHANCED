...
class AdvancedFinancialAnalysis:
    ...
    def load_ratio_context(self, data_by_year: Dict, ratios_by_year: Dict) -> None:
    ...
    def dynamic_failure_prediction(self, data_by_year: Dict, ratios_by_year: Dict) -> Dict:
        ...
        s = 0.0
        if len(z_scores) >= 2:
            if (z_scores[-1] - z_scores[0]) < -0.5:
                s += 0.25
            if z_scores[-1] < 1.80:
                s += 0.30
        if debt_ebitda:
            avg = float(np.mean(debt_ebitda))
            s += 0.30 if avg > 5 else (0.15 if avg > 3 else 0.0)
        if interest_cov:
            avg = float(np.mean(interest_cov))
            s += 0.25 if avg < 1.5 else (0.10 if avg < 3 else 0.0)
        p3 = min(s, 0.90)
        p5 = min(s * 1.3, 0.95)
        return {
            'failure_prob_3y': p3,
    ...
    def growth_sustainability_grade(self, data_by_year: Dict, ratios_by_year: Dict) -> Dict:
        ...
        if isinstance(roic, (int, float)):
            score += 30 if roic > 0.15 else (20 if roic > 0.10 else 10)
        if isinstance(retention, (int, float)):
            score += 25 if retention > 0.70 else (15 if retention > 0.50 else 0)
        if isinstance(sgr, (int, float)):
            score += 30 if actual_growth <= sgr else (15 if actual_growth <= sgr * 1.5 else 0)
        ...
        if len(years) >= 2:
            ...
            if debt_growth > actual_growth * 1.5:
                debt_warning = True
                score -= 20

        grade = 'A' if score >= 80 else ('B' if score >= 60 else ('C' if score >= 40 else 'D'))
        return {
            'sustainability_score': max(score, 0),
            'grade': grade,
            'sgr_internal': sgr,
    ...
    def ai_investment_quality_score(self, data_by_year: Dict, ratios_by_year: Dict, investment_score: float, economic_spread: float, fcf_yield: float) -> Dict:
        ...
        score = 0
        if economic_spread and economic_spread > 0.10:
            score += 25
        elif economic_spread and economic_spread > 0.05:
            score += 15
...
def generate_comprehensive_forecast(data_by_year: Dict, ratios_by_year: Dict, sgr: float, wacc: float, roic: float) -> Dict:
...
def generate_ai_insights(data_by_year: Dict, ratios_by_year: Dict, investment_score: float, economic_spread: float, fcf_yield: float) -> Dict:
    analyzer = AdvancedFinancialAnalysis()
    analyzer.load_ratio_context(data_by_year, ratios_by_year)
    insights = {
