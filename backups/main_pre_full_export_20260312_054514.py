...
class SECFinancialSystem:
    ...
    def _build_section_card(self, parent, title, subtitle=None):
        ...
        head = tk.Frame(card, bg=PALETTE['surface_alt'], height=34)
        head.pack(fill='x')
        ...
        title_label.pack(fill='x', pady=(5 if subtitle else 7, 0))
        if subtitle:
            tk.Label(
                head,
                text=subtitle,
    ...
    def _build_left(self, parent):
        ...
        universe_body.grid_rowconfigure(2, weight=1)
        ...
        self.company_entry.grid(row=1, column=0, sticky='ew', pady=(0, 6))
        ...
        self.add_company_btn.grid(row=1, column=1, padx=(10, 0), sticky='ew')
        self.add_company_btn.configure(padx=6, pady=8)
        ...
        self.companies_listbox = tk.Listbox(universe_body, height=5, selectmode=tk.EXTENDED)
        self._style_listbox_widget(self.companies_listbox)
        self.companies_listbox.grid(row=3, column=0, columnspan=2, sticky='nsew', pady=(0, 6))
        ...
        btns_frame.grid(row=4, column=0, columnspan=2, sticky='ew')
        ...
        years_frame.pack(fill='x', pady=(4, 6))
        ...
        self.fetch_btn.pack(fill='x', pady=(4, 3))
        self.fetch_btn.configure(padx=6, pady=7)
        ...
        self.quick_load_btn.configure(padx=6, pady=6)
        ...
        self.quick_export_btn.configure(padx=6, pady=6)

        status_sep = tk.Frame(actions_body, bg=PALETTE['border'], height=1)
        status_sep.pack(fill='x', pady=(6, 4))
        ...
        self.loading_indicator_canvas = tk.Canvas(actions_body, width=220, height=6, bg=PALETTE['surface_alt'], highlightthickness=0)
    ...
    def _build_right(self, parent):
        ...
        workspace_bar.grid_columnconfigure(0, weight=1)
        workspace_bar.grid_columnconfigure(1, weight=0)

        left_meta = tk.Frame(workspace_bar, bg=PALETTE['surface_alt'])
        left_meta.grid(row=0, column=0, sticky='ew', padx=10, pady=6)
        left_meta.grid_columnconfigure(1, weight=1)
        ...
        self.workspace_context_label.grid(row=0, column=0, sticky='w')

        chips_row = tk.Frame(left_meta, bg=PALETTE['surface_alt'])
        chips_row.grid(row=0, column=1, sticky='w', padx=(14, 0))
        ...
        self.workspace_hint_label.grid(row=0, column=1, sticky='e', padx=10, pady=6)
    ...
    def _get_comparison_source_snapshot(self, preferred_current=None):
        source = {}
        for key, data in (self.multi_company_data or {}).items():
    ...
    def display_raw_data(self):
        ...
        sector_display = self._sector_profile_to_display(ci.get('sector') or ((self.current_data.get('sector_gating', {}) if self.current_data else {}) or {}).get('profile'))
        if self.current_lang == 'ar':
        ...
        def _safe_display_fallback(year, ratio_key):
            ...
            if ratio_key == 'cost_of_debt':
                ...
                if interest is not None and total_debt not in (None, 0):
                    ...
                    if 0 < cod <= 0.5:
                        return cod
                sector_profile = ((self.current_data.get('sector_gating', {}) if self.current_data else {}) or {}).get('profile', 'industrial')
                defaults = {
                    'bank': 0.035,
                    'insurance': 0.045,
                    'industrial': 0.055,
            ...
            if ratio_key == 'ev_ebitda':
                ...
                if ev not in (None, 0) and ebitda not in (None, 0):
                    ...
                    plausible = [c for c in cands if c not in (None, 0) and 0.5 <= (abs(float(ev)) / abs(float(c))) <= 200.0]
                    denom = min(plausible, key=lambda c: abs((abs(float(ev)) / abs(float(c))) - 20.0)) if plausible else None
        ...
        try:
            ...
            loaded = {
                ...
                'institutional_outputs': {
                    'classification': {
                        'primary_profile': imported_meta.get('sector_profile') or 'unknown',
                        'confidence': imported_meta.get('confidence'),
                    },
                    'classifier_diagnostics': {
                        'sector': imported_meta.get('sector_profile') or 'unknown',
    ...
    def _build_investor_verdict_df(self, years, ratios_by_year, gate_issues, critical_df, forecasts_df):
        try:
            import pandas as pd
            import re
    ...
    # ---------- Forecasts & Plots ----------
    def _extract_metric_history(self, data_by_year, metric_aliases):
        hist = []
    ...
    def _build_metric_forecast_enhanced(self, data_by_year, metric, aliases, years_forward=10):
        hist = self._extract_metric_history(data_by_year, aliases)
        out = {
            'metric': metric,
            'forecast': {},
        ...
        out.update({
            'forecast': blended,
            'method': 'enhanced_hybrid_structural',
            'bull_band': band_base,
    ...
    def _build_forecast_bundle(self, data_by_year, years_forward=10):
        revenue = self._build_metric_forecast_enhanced(
            data_by_year=data_by_year,
            metric='Revenues',
            aliases=['Revenues', 'Revenue', 'SalesRevenueNet', 'TotalRevenue'],
    ...
    def _build_forecast_figure(self, data_by_year, bundle):
        rev_hist = self._extract_metric_history(data_by_year, ['Revenues', 'Revenue', 'SalesRevenueNet', 'TotalRevenue'])
