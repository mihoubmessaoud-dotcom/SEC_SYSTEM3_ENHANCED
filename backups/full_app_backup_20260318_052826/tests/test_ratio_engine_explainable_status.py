...
class RatioEngineExplainableStatusTests(unittest.TestCase):
    ...
    def test_sector_not_applicable_maps_to_data_not_applicable(self):
        eng = RatioEngine()
        rev = {"value": 100.0, "tag": "Revenues", "period_end": "2024-12-31"}
        cogs = {"value": None, "tag": None, "selection_reason": "not_applicable_for_sector:bank"}
        c = eng._compute_gross_margin(rev, cogs)
        out = eng._finalize_contract("gross_margin", c)
        self.assertEqual(out["status"], "NOT_COMPUTABLE")
        self.assertEqual(out["reason"], "DATA_NOT_APPLICABLE")
